...
// Dispatch for the *batched* two-instance MtA: a single OT extension covers
// both multiplications, so this costs fewer rounds than two MtAFunction calls.
// The verifier plays the sender; the prover plays the receiver.
template <bool verifier> struct MtABatchFunction {};

template <> struct MtABatchFunction<true> {
  static bool run(BIGNUM *const o1, BIGNUM *const o2,
                  emp::IKNP<EmpWrapper<>> &ot, const BIGNUM *const a1,
                  const BIGNUM *const a2, const BIGNUM *const p,
                  BN_CTX *const ctx, SenderEntry &e1, SenderEntry &e2) noexcept {
    return MtA::play_sender_inplace_iknp_2(o1, o2, ot, a1, a2, p, ctx, e1, e2);
  }
};

template <> struct MtABatchFunction<false> {
  static bool run(BIGNUM *const o1, BIGNUM *const o2,
                  emp::IKNP<EmpWrapper<>> &ot, const BIGNUM *const b1,
                  const BIGNUM *const b2, const BIGNUM *const p,
                  BN_CTX *const ctx, ReceiverEntry &e1,
                  ReceiverEntry &e2) noexcept {
    return MtA::play_receiver_inplace_iknp_2(o1, o2, ot, b1, b2, p, ctx, e1, e2);
  }
};
...
// This function acts as a dispatch for MtA across two values, a1 and a2. The
// two multiplications are run over a SINGLE batched oblivious transfer (see
// MtABatchFunction), which costs fewer rounds than two separate MtA calls. The
// produced shares are then added together and stored in `out`.
...
static bool vector_mta_2(BIGNUM *const out, BIGNUM *const a1, BIGNUM *const a2,
                         ...
                         ET &entry) noexcept {
  if (!(out && a1 && a2 && first && second && p && ctx)) {
    return false;
  }

  // The batched MtA runs both instances simultaneously, so it needs a second
  // (independent) entry's worth of scratch buffers. `entry` is reused across the
  // ectf as before; `entry2` is local to this call.
  auto entry2 = MtABuild<verifier>::build(p, ctx);
  if (!MtABatchFunction<verifier>::run(a1, a2, ot, first, second, p, ctx, entry,
                                       entry2)) {
    return false;
  }

  return BN_mod_add(out, a1, a2, p, ctx);
}
...
scale_plus_share_impl(BIGNUM *out, OTType &ot, const BIGNUM *const val,
  ...
  // We rely upon both parties supplying their inputs in a different order.
  ...
  // This is the (batched) MtA call.
  if (!vector_mta_2<verifier>(tmp_1, tmp_2, tmp_3, first, second, p, ctx, ot,
                              et)) {
    return false;
  }
  ...
  *size_of_p = BN_num_bytes(p);
  ...
  if (track_bandwidth) {
    ...
    *bandwidth = wrapper.counter;
    if (rounds) {
      *rounds = wrapper.get_round_counter();
    }
  }
...
bool ECtF::ectf_produce_bignum(BIGNUM *const out, SSL *const ssl,
  ...
  if (verifier) {
    return ectf_internal<true>(out, ssl, x_secret, y_secret, curve_id,
                               size_of_p, track_bandwidth, bandwidth, rounds);
  } else {
