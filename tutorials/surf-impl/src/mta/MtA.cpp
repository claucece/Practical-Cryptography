...
bool MtA::play_receiver_inplace_iknp_2(
    BIGNUM *const out1, BIGNUM *const out2, emp::IKNP<EmpWrapper<>> &ot,
    const BIGNUM *const b1, const BIGNUM *const b2, const BIGNUM *const q,
    BN_CTX *const ctx, ReceiverEntry &re1, ReceiverEntry &re2) noexcept {
  if (!out1 || !out2 || !b1 || !b2 || !q || !ctx) {
    return false;
  }
  ...
  ReceiverEntry *const res[2] = {&re1, &re2};
  const BIGNUM *const bs[2] = {b1, b2};
  BIGNUM *const outs[2] = {out1, out2};
  BIGNUM *sigmas[2] = {nullptr, nullptr};

  // Per-instance preparation: generate t,v, expand the choice bits, serialise v
  // and sample sigma. This is exactly what play_receiver_inplace_internal does
  // per call, but we hold the results so the OT can be batched.
  for (int i = 0; i < 2; ++i) {
    auto &re = *res[i];
    if (!MtA::generate_t_and_v_inplace(*bs[i], *q, re.t, re.v, *ctx) ||
        !MtA::expand_t_to_ot_width_inplace(ots_per_bignum, re.t,
                                           re.t_extended) ||
        !MtA::serialise_bignums_inplace(re.v_serialised, re.v, size_of_each)) {
      return false;
    }
    sigmas[i] = BN_CTX_get(ctx);
    if (!sigmas[i] || !BN_rand_range_ex(sigmas[i], 1, q) ||
        !BN_bn2bin_padded(re.sigma_serialised.data(), size_of_each,
                          sigmas[i])) {
      return false;
    }
  }

  // Concatenate both instances' choice vectors and run a SINGLE OT extension.
  // Both instances share the same q, so their choice vectors are equal length.
  const auto half = re1.t_extended.size();
  if (half != re2.t_extended.size()) {
    return false;
  }
  bssl::Array<bool> t_all;
  EmpBlockOwningSpan z_all;
  if (!t_all.Init(half * 2) || !z_all.init(half * 2)) {
    return false;
  }
  std::memcpy(t_all.data(), re1.t_extended.data(), half * sizeof(bool));
  std::memcpy(t_all.data() + half, re2.t_extended.data(), half * sizeof(bool));

  if (!MtA::receiver_ot(ot, z_all, t_all)) {
    return false;
  }

  // Split the OT outputs back into each instance's buffer.
  std::memcpy(re1.z.data(), z_all.data(), half * sizeof(emp::block));
  std::memcpy(re2.z.data(), z_all.data() + half, half * sizeof(emp::block));

  // Send v and sigma for both instances, in instance order (the sender reads
  // them back in the same order).
  auto *const ssl = ot.io->get_ssl();
  for (int i = 0; i < 2; ++i) {
    auto &re = *res[i];
    const auto v_bytes = static_cast<int>(re.v_serialised.size());
    const auto s_bytes = static_cast<int>(re.sigma_serialised.size());
    if (SSL_write(ssl, re.v_serialised.data(), v_bytes) != v_bytes ||
        SSL_write(ssl, re.sigma_serialised.data(), s_bytes) != s_bytes) {
      return false;
    }
  }

  // Finish: out_i = <z_i, v_i> + sigma_i.
  for (int i = 0; i < 2; ++i) {
    auto &re = *res[i];
    BN_zero(outs[i]);
    BIGNUM *const tmp = BN_CTX_get(ctx);
    if (!tmp ||
        !MtA::deserialise_inner_product(tmp, re.z, re.v, size_of_each, q,
                                        ctx) ||
        !BN_mod_add(outs[i], tmp, sigmas[i], q, ctx)) {
      return false;
    }
  }
  return true;
...
bool MtA::play_sender_inplace_iknp_2(
    BIGNUM *const out1, BIGNUM *const out2, emp::IKNP<EmpWrapper<>> &ot,
    const BIGNUM *const a1, const BIGNUM *const a2, const BIGNUM *const q,
    BN_CTX *const ctx, SenderEntry &se1, SenderEntry &se2) noexcept {
  if (!out1 || !out2 || !a1 || !a2 || !q || !ctx) {
    return false;
  }
  ...
  SenderEntry *const ses[2] = {&se1, &se2};
  const BIGNUM *const as[2] = {a1, a2};
  BIGNUM *const outs[2] = {out1, out2};

  // Per-instance preparation: sample delta and produce the OT message pairs.
  for (int i = 0; i < 2; ++i) {
    auto &se = *ses[i];
    if (!MtA::generate_random_vector_inplace(se.delta, *q) ||
        !MtA::produce_sender_pairs_inplace(se.delta, as[i], size_of_each,
                                           se.add_pack, se.sub_pack, q, ctx)) {
      return false;
    }
  }

  // Concatenate both instances' OT pairs and run a SINGLE OT extension. The
  // block order must match the receiver's concatenated choice vector.
  const auto half = se1.sub_pack.size();
  if (half != se2.sub_pack.size() || half != se1.add_pack.size() ||
      half != se2.add_pack.size()) {
    return false;
  }
  PackArray sub_all, add_all;
  if (!sub_all.init(half * 2) || !add_all.init(half * 2)) {
    return false;
  }
  std::memcpy(sub_all.data(), se1.sub_pack.data(), half);
  std::memcpy(sub_all.data() + half, se2.sub_pack.data(), half);
  std::memcpy(add_all.data(), se1.add_pack.data(), half);
  std::memcpy(add_all.data() + half, se2.add_pack.data(), half);

  const auto *const b0 = reinterpret_cast<const emp::block *>(sub_all.data());
  const auto *const b1 = reinterpret_cast<const emp::block *>(add_all.data());
  assert(sub_all.size() % sizeof(emp::block) == 0);
  const auto size_in_blocks = sub_all.size() / sizeof(emp::block);

  if (!sender_ot_internal(ot, b0, b1, static_cast<int64_t>(size_in_blocks))) {
    return false;
  }

  // Read v and sigma for both instances, in instance order (matches the order
  // the receiver wrote them).
  auto *const ssl = ot.io->get_ssl();
  BIGNUM *sigmas[2] = {nullptr, nullptr};
  const auto vals_size = static_cast<int>(n * size_of_each);
  const auto sigma_size = static_cast<int>(size_of_each);
  for (int i = 0; i < 2; ++i) {
    ...
    while (read_data < vals_size) {
      const auto recv =
          SSL_read(ssl, se.vals.data() + read_data, vals_size - read_data);
      if (recv <= 0) {
        return false;
      }
      read_data += recv;
    ...
    sigmas[i] = BN_CTX_get(ctx);
    if (!sigmas[i] ||
        SSL_read(ssl, se.sigma_v.data(), sigma_size) != sigma_size ||
        !BN_bin2bn(se.sigma_v.data(), se.sigma_v.size(), sigmas[i])) {
      return false;
    }
  }

  // Finish: out_i = -<vals_i, delta_i> - sigma_i.
  for (int i = 0; i < 2; ++i) {
    auto &se = *ses[i];
    BIGNUM *const tmp = BN_CTX_get(ctx);
    BIGNUM *const zero = BN_CTX_get(ctx);
    BIGNUM *const share = BN_CTX_get(ctx);
    if (!tmp || !zero || !share ||
        !MtA::deserialise_inner_product(tmp, se.vals, se.delta, size_of_each, q,
                                        ctx) ||
        !BN_mod_sub(share, zero, tmp, q, ctx) ||
        !BN_mod_sub(outs[i], share, sigmas[i], q, ctx)) {
      return false;
    }
  }
  return true;
