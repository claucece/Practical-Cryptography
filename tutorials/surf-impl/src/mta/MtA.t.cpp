...
TEST_CASE("full_ot") {
  ...
  // groups and curves.
  constexpr auto nr_curves = 3;
  static constexpr uint16_t curves[]{SSL_CURVE_SECP256R1,
  ...
  auto build_experm = [&]() {
    ...
    for (unsigned i = 0; i < set.groups.size(); i++) {
      ...
      // NB: these are stored in bssl::UniquePtr<BIGNUM>, so they must be
      // independently owned (BN_new). Allocating them with BN_CTX_get() would
      // make the BN_CTX own them too, double-freeing at ExpermSet destruction.
      set.coords[i].reset(BN_new());
...
//! [MtABatchedOTCorrectness]
TEST_CASE("batched_ot_2_correctness") {
  // Two independent MtA instances run over a single batched OT must each
  // reconstruct correctly: sout_i + rout_i == a_i * b_i (mod q).
  auto context = CreateContextWithTestCertificate(TLS_method());
  REQUIRE(context);
  ...
  bssl::UniquePtr<BN_CTX> sbn_ctx(BN_CTX_new()), rbn_ctx(BN_CTX_new());
  REQUIRE(sbn_ctx);
  REQUIRE(rbn_ctx);
  bssl::BN_CTXScope sscope(sbn_ctx.get()), rscope(rbn_ctx.get());

  // Sender holds a1,a2; receiver holds b1,b2.
  BIGNUM *a1 = BN_CTX_get(sbn_ctx.get()), *a2 = BN_CTX_get(sbn_ctx.get());
  BIGNUM *b1 = BN_CTX_get(rbn_ctx.get()), *b2 = BN_CTX_get(rbn_ctx.get());
  REQUIRE(a1); REQUIRE(a2); REQUIRE(b1); REQUIRE(b2);
  REQUIRE(BN_rand_range_ex(a1, 1, q));
  REQUIRE(BN_rand_range_ex(a2, 1, q));
  REQUIRE(BN_rand_range_ex(b1, 1, q));
  REQUIRE(BN_rand_range_ex(b2, 1, q));

  BIGNUM *sout1 = BN_CTX_get(sbn_ctx.get()), *sout2 = BN_CTX_get(sbn_ctx.get());
  BIGNUM *rout1 = BN_CTX_get(rbn_ctx.get()), *rout2 = BN_CTX_get(rbn_ctx.get());
  REQUIRE(sout1); REQUIRE(sout2); REQUIRE(rout1); REQUIRE(rout2);
  ...
  const auto sender_code = [&]() {
    ...
    emp::IKNP<EmpWrapper<>> ot(&wrapper, true);
    SenderEntry se1, se2;
    REQUIRE(MtA::initialise_entries_for_sender(q, se1, sbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_sender(q, se2, sbn_ctx.get()));
    REQUIRE(MtA::play_sender_inplace_iknp_2(sout1, sout2, ot, a1, a2, q,
                                            sbn_ctx.get(), se1, se2));
  };
  ...
  std::thread t(sender_code);
  {
    ...
    EmpWrapper<> wrapper{receiver_socket.get_ssl_object()};
    emp::IKNP<EmpWrapper<>> ot(&wrapper, true);
    ReceiverEntry re1, re2;
    REQUIRE(MtA::initialise_entries_for_receiver(q, re1, rbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_receiver(q, re2, rbn_ctx.get()));
    REQUIRE(MtA::play_receiver_inplace_iknp_2(rout1, rout2, ot, b1, b2, q,
                                              rbn_ctx.get(), re1, re2));
  }
  t.join();

  std::scoped_lock<std::mutex> lock(m);
  // Instance 1: sout1 + rout1 == a1 * b1.
  BIGNUM *prod = BN_CTX_get(rbn_ctx.get()), *sum = BN_CTX_get(rbn_ctx.get());
  REQUIRE(prod); REQUIRE(sum);
  REQUIRE(BN_mod_mul(prod, a1, b1, q, rbn_ctx.get()));
  REQUIRE(BN_mod_add(sum, sout1, rout1, q, rbn_ctx.get()));
  CHECK(BN_cmp(prod, sum) == 0);
  // Instance 2: sout2 + rout2 == a2 * b2.
  REQUIRE(BN_mod_mul(prod, a2, b2, q, rbn_ctx.get()));
  REQUIRE(BN_mod_add(sum, sout2, rout2, q, rbn_ctx.get()));
  CHECK(BN_cmp(prod, sum) == 0);
}
//! [MtABatchedOTCorrectness]

//! [MtABatchedOTFewerRounds]
TEST_CASE("batched_ot_2_fewer_rounds") {
  // The batched 2-instance call must use strictly fewer OT rounds (outbound
  // flushes, measured receiver-side) than two separate single calls. This is
  // the whole point of the batch: amortise the OT extension.
  auto context = CreateContextWithTestCertificate(TLS_method());
  REQUIRE(context);
  ...
  BIGNUM *s1 = BN_CTX_get(sbn_ctx.get()), *s2 = BN_CTX_get(sbn_ctx.get());
  BIGNUM *r1 = BN_CTX_get(rbn_ctx.get()), *r2 = BN_CTX_get(rbn_ctx.get());

  std::mutex m;
  std::size_t single_rounds = 0, batch_rounds = 0;
  ...
  const auto sender_code = [&]() {
    ...
    EmpWrapper<> wrapper{sender.get_ssl_object()};
    emp::IKNP<EmpWrapper<>> ot(&wrapper, true);
    SenderEntry se0, se1, se2, se3, se4;
    REQUIRE(MtA::initialise_entries_for_sender(q, se0, sbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_sender(q, se1, sbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_sender(q, se2, sbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_sender(q, se3, sbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_sender(q, se4, sbn_ctx.get()));
    // Warm up: trigger the one-time base-OT setup so it doesn't pollute the
    // per-phase round counts.
    REQUIRE(MtA::play_sender_inplace_iknp(s1, ot, a1, q, sbn_ctx.get(), se0));
    // Phase A: two separate single calls.
    REQUIRE(MtA::play_sender_inplace_iknp(s1, ot, a1, q, sbn_ctx.get(), se1));
    REQUIRE(MtA::play_sender_inplace_iknp(s2, ot, a2, q, sbn_ctx.get(), se2));
    // Phase B: one batched call.
    REQUIRE(MtA::play_sender_inplace_iknp_2(s1, s2, ot, a1, a2, q,
                                            sbn_ctx.get(), se3, se4));
  };
  ...
  std::thread t(sender_code);
  {
    ...
    EmpWrapper<> wrapper{receiver_socket.get_ssl_object()};
    emp::IKNP<EmpWrapper<>> ot(&wrapper, true);
    ReceiverEntry re0, re1, re2, re3, re4;
    REQUIRE(MtA::initialise_entries_for_receiver(q, re0, rbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_receiver(q, re1, rbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_receiver(q, re2, rbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_receiver(q, re3, rbn_ctx.get()));
    REQUIRE(MtA::initialise_entries_for_receiver(q, re4, rbn_ctx.get()));

    // Warm up: trigger the one-time base-OT setup so it doesn't pollute the
    // per-phase round counts.
    REQUIRE(MtA::play_receiver_inplace_iknp(r1, ot, b1, q, rbn_ctx.get(), re0));
    const auto c0 = wrapper.get_round_counter();
    // Phase A: two separate single calls.
    REQUIRE(MtA::play_receiver_inplace_iknp(r1, ot, b1, q, rbn_ctx.get(), re1));
    REQUIRE(MtA::play_receiver_inplace_iknp(r2, ot, b2, q, rbn_ctx.get(), re2));
    const auto c1 = wrapper.get_round_counter();
    // Phase B: one batched call.
    REQUIRE(MtA::play_receiver_inplace_iknp_2(r1, r2, ot, b1, b2, q,
                                              rbn_ctx.get(), re3, re4));
    const auto c2 = wrapper.get_round_counter();
    single_rounds = c1 - c0;
    batch_rounds = c2 - c1;
  }
  t.join();
  ...
  std::scoped_lock<std::mutex> lock(m);
  MESSAGE("single (2 calls) rounds=" << single_rounds
                                     << " batched rounds=" << batch_rounds);
  CHECK(batch_rounds < single_rounds);
}
//! [MtABatchedOTFewerRounds]
