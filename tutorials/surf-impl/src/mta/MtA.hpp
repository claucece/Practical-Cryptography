...
/**
 play_sender_inplace_iknp_2. This function runs two MtA instances from the
perspective of the `sender` over a *single* batched oblivious transfer. The
sender inputs `a1`, `a2` (elements mod `q`) and receives shares `out1`, `out2`
such that `out1` + (receiver's beta1) = a1 * b1 (mod `q`) and likewise for the
second instance. The two instances are independent; batching only amortises the
OT extension so this costs fewer rounds than two separate calls to
play_sender_inplace_iknp. `se1` and `se2` are the (separate) sender entries for
the two instances. Returns true on success, false otherwise.
**/
bool play_sender_inplace_iknp_2(BIGNUM *const out1, BIGNUM *const out2,
                                emp::IKNP<EmpWrapper<>> &ot,
                                const BIGNUM *const a1, const BIGNUM *const a2,
                                const BIGNUM *const q, BN_CTX *const ctx,
                                SenderEntry &se1, SenderEntry &se2) noexcept;
...
/**
 play_receiver_inplace_iknp_2. This function runs two MtA instances from the
perspective of the `receiver` over a *single* batched oblivious transfer. The
receiver inputs `b1`, `b2` (elements mod `q`) and receives shares `out1`, `out2`
such that (sender's alpha1) + `out1` = a1 * b1 (mod `q`) and likewise for the
second instance. The two instances are independent; batching only amortises the
OT extension so this costs fewer rounds than two separate calls to
play_receiver_inplace_iknp. `re1` and `re2` are the (separate) receiver entries
for the two instances. Returns true on success, false otherwise.
**/
bool play_receiver_inplace_iknp_2(BIGNUM *const out1, BIGNUM *const out2,
                                  emp::IKNP<EmpWrapper<>> &ot,
                                  const BIGNUM *const b1, const BIGNUM *const b2,
                                  const BIGNUM *const q, BN_CTX *const ctx,
                                  ReceiverEntry &re1,
                                  ReceiverEntry &re2) noexcept;
