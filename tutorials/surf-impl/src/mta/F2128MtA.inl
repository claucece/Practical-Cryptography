...
inline emp::block F2128_MTA::arr_to_block(const std::array<uint8_t, 16> &in) noexcept {
...
inline std::array<uint8_t, 16> F2128_MTA::block_to_arr(const emp::block in) noexcept {
...
inline emp::block F2128_MTA::mul(const emp::block a, const emp::block b) noexcept {
...
inline emp::block F2128_MTA::inv(const emp::block a) noexcept {
...
inline std::unique_ptr<F2128_MTA::AType> F2128_MTA::generate_a() noexcept {
  auto a = new AType();
  Util::generate_random_bytes<sizeof(AType)>(a->data());
  return std::unique_ptr<AType>(a);
}

inline std::unique_ptr<F2128_MTA::AlphaType>
...
inline F2128_MTA::BetaType F2128_MTA::generate_beta() noexcept {
  BetaType beta;
  Util::generate_random_bytes<sizeof(beta)>(&beta);
  return beta;
}

inline F2128_MTA::BType F2128_MTA::produce_b(const GadgetType &gadget,
...
inline F2128_MTA::GadgetType F2128_MTA::generate_gadget_r() noexcept {
  GadgetType g_r;
  Util::generate_random_bytes<sizeof(GadgetType)>(g_r.data());
  return g_r;
}

inline F2128_MTA::GType F2128_MTA::generate_g(const GadgetType &g_r) noexcept {
  GType g;
...
inline emp::block F2128_MTA::dot_product(const emp::block *const gamma,
...
inline F2128_MTA::EncodeType F2128_MTA::encode(const emp::block in,
                                        const GadgetType &gadget) noexcept {
  EncodeType out;
...
inline F2128_MTA::TType F2128_MTA::generate_pads() noexcept {
  TType elems;
  Util::generate_random_bytes<sizeof(elems)>(elems.data());
  return elems;
}

inline std::unique_ptr<F2128_MTA::BatchedTType>
...
inline emp::block F2128_MTA::generate_alpha_hat() noexcept {
  emp::block alpha_hat;
  Util::generate_random_bytes<sizeof(alpha_hat)>(&alpha_hat);
  return alpha_hat;
}

inline std::unique_ptr<F2128_MTA::BatchedOTType>
...
inline F2128_MTA::OTType F2128_MTA::prepare_pairs(const emp::block alpha,
...
inline F2128_MTA::OTOutType F2128_MTA::get_sender_out(const TType &ts) noexcept {
  OTOutType out;
...
inline std::unique_ptr<F2128_MTA::BatchedOTOutType>
...
inline F2128_MTA::ChoiceBitType
...
inline std::unique_ptr<F2128_MTA::BatchedChoiceBitType>
...
F2128_MTA::generate_batched_randomness(SSL &ssl, const bool sender) noexcept {
  ...
  // N.B These are multi-kilobyte transfers, so a bare SSL_read/SSL_write can
  // return a short count (data split across TLS records) or SSL_ERROR_WANT_READ
  // over a real network; ignoring that part-fills the buffer and desynchronises
  // the 2PC stream. Route them through Util::process_data, which loops and
  // poll-retries — the same way compute_mult_shares/check_consistency_batched
  // already do.
  if (sender) {
    Util::process_data(&ssl, reinterpret_cast<char *>(&val[1]),
                       sizeof(val[1]), SSL_read);
    memcpy(&val[0], randomness, sizeof(randomness));
    Util::process_data(&ssl, reinterpret_cast<const char *>(&val[0]),
                       sizeof(val[0]), SSL_write);
  } else {
    memcpy(&val[1], randomness, sizeof(randomness));
    Util::process_data(&ssl, reinterpret_cast<const char *>(&randomness),
                       sizeof(randomness), SSL_write);
    Util::process_data(&ssl, reinterpret_cast<char *>(&val[0]),
                       sizeof(val[0]), SSL_read);
  }
  return val;
}

inline F2128_MTA::RandomType
...
inline bool F2128_MTA::check_consistency(SSL &ssl, const OTOutType &t_b,
...
inline bool F2128_MTA::check_consistency_batched(SSL &ssl, const BatchedOTOutType &t_b,
                                          const ChiType &chi,
                                          const BetaType &beta) noexcept {
...
inline bool F2128_MTA::check_consistency_batched(
  ...
  if (!Util::process_data(&ssl, reinterpret_cast<const char *>(&correct),
                          sizeof(correct), SSL_write)) {
    return false;
  }
...
inline bool F2128_MTA::prove_consistency_batched(SSL &ssl, const AType &a_tilde,
                                          const AType &a_hat,
  ...
  // r is ~12 KB; the receiver reads it with Util::process_data, so the sender
  // must write it robustly too (a bare SSL_write can short-count / WANT_WRITE
  // and desync the stream — exactly the bug that surfaces downstream as a
  // garbage `worked` byte and an implausible 2PC frame).
  static_assert(sizeof(BatchedRType) < SSL3_RT_MAX_PLAIN_LENGTH);
  if (!Util::process_data(&ssl, reinterpret_cast<const char *>(r->data()),
                          sizeof(BatchedRType), SSL_write)) {
    return false;
  }

  bool worked;
  if (!Util::process_data(&ssl, reinterpret_cast<char *>(&worked),
                          sizeof(worked), SSL_read)) {
    return false;
  }
  return worked;
}

inline bool F2128_MTA::prove_consistency(SSL &ssl, const emp::block alpha,
                                  const emp::block alpha_hat,
                                  const OTOutType &t_a,
...
inline emp::block F2128_MTA::compute_share(const unsigned upper,
...
inline F2128_MTA::GadgetType F2128_MTA::generate_and_send_gadget(SSL &ssl) noexcept {
  auto out = generate_gadget_r();
  // GadgetType is several KB; use the looping/retrying helper (see
  // generate_batched_randomness) rather than a bare SSL_write.
  Util::process_data(&ssl, reinterpret_cast<const char *>(&out), sizeof(out),
                     SSL_write);
  return out;
}

inline F2128_MTA::GadgetType F2128_MTA::receive_gadget(SSL &ssl) noexcept {
  GadgetType out;
  Util::process_data(&ssl, reinterpret_cast<char *>(&out), sizeof(out),
                     SSL_read);
  return out;
}
...
F2128_MTA::generate_shares_repeated(OTType &socket, SSL &ssl,
  ...
  for (unsigned i = 2; i < num_powers; i += 2) {
...
inline F2128_MTA::ReceiverType::ReceiverType(const GadgetType &gadget) noexcept
...
inline F2128_MTA::SenderType::SenderType(const GadgetType &) noexcept
...
inline void F2128_MTA::ReceiverType::generate_batched_randomness(SSL &ssl) noexcept {
  chi = F2128_MTA::generate_batched_randomness(ssl, false);
}

inline void F2128_MTA::SenderType::generate_batched_randomness(SSL &ssl) noexcept {
  chi = F2128_MTA::generate_batched_randomness(ssl, true);
}

inline bool F2128_MTA::SenderType::is_consistent(SSL &ssl) noexcept {
  return F2128_MTA::prove_consistency_batched(ssl, *a_tilde, *a_hat, *t_a, chi);
}

inline bool F2128_MTA::ReceiverType::is_consistent(SSL &ssl) noexcept {
...
F2128_MTA::generate_shares_batched(OTType &socket, SSL &ssl,
  ...
  if (use_multiplicative_shares) {
    ...
    for (unsigned i = 0; i < num_powers; i += 2) {
      compute_squares(i);
    }
...
inline F2128_MTA::ShareType F2128_MTA::generate_shares_prover_batched(
...
inline F2128_MTA::ShareType F2128_MTA::generate_shares_verifier_batched(
    SSL &ssl, const std::array<uint8_t, 16> &in, uint64_t &bandwidth) noexcept {
