...
bool ectf_produce_bignum(BIGNUM *const out, SSL *const ssl,
                         ...
                         uint64_t* const bandwidth = nullptr,
                         uint64_t* const rounds = nullptr) noexcept;
...
this function returns false if:
 ...
 @param[in] bandwidth: the location to store the bandwidth. Can be nullptr if
 track_bandwidth is false.
 @param[out] rounds: the location to store the number of communication rounds
 (outbound messages). Can be nullptr, in which case it is not written, even if
 track_bandwidth is true.
**/
