...
TEST_CASE("share_inversion") {
  ...
  constexpr auto curve_id = NID_secp224r1;
...
//! [ECtFBatchedRounds]
TEST_CASE("ectf_batched_round_count") {
  // Regression guard for the batched MtA: the prover-side ECtF round count must
  // stay below the pre-batch baseline. ECtF runs two vector_mta_2 calls (in
  // share_inversion and compute_lambda); batching each saves an OT extension,
  // so the prover should now use markedly fewer rounds than the unbatched ~11.
  constexpr auto curve_id = SSL_CURVE_SECP256R1;
  auto context = CreateContextWithTestCertificate(TLS_method());
  REQUIRE(context);
  ...
  bssl::UniquePtr<EC_POINT> rpoint(EC_POINT_new(group.get())),
      spoint(EC_POINT_new(group.get()));
  do {
    REQUIRE(Util::RandomECPoint(group.get(), rpoint.get(), rbn_ctx.get()));
    REQUIRE(Util::RandomECPoint(group.get(), spoint.get(), sbn_ctx.get()));
  } while (EC_POINT_cmp(group.get(), rpoint.get(), spoint.get(),
                        sbn_ctx.get()) == 0);

  BIGNUM *rx = BN_CTX_get(rbn_ctx.get()), *ry = BN_CTX_get(rbn_ctx.get());
  BIGNUM *sx = BN_CTX_get(sbn_ctx.get()), *sy = BN_CTX_get(sbn_ctx.get());
  REQUIRE(EC_POINT_get_affine_coordinates_GFp(group.get(), rpoint.get(), rx, ry,
                                              rbn_ctx.get()));
  REQUIRE(EC_POINT_get_affine_coordinates_GFp(group.get(), spoint.get(), sx, sy,
                                              sbn_ctx.get()));

  const auto size = (EC_GROUP_get_degree(group.get()) + 7) / 8;
  bssl::Array<uint8_t> s_secret_x, s_secret_y, r_secret_x, r_secret_y;
  REQUIRE(s_secret_x.Init(size)); REQUIRE(s_secret_y.Init(size));
  REQUIRE(r_secret_x.Init(size)); REQUIRE(r_secret_y.Init(size));
  REQUIRE(BN_bn2bin_padded(s_secret_x.data(), size, sx));
  REQUIRE(BN_bn2bin_padded(s_secret_y.data(), size, sy));
  REQUIRE(BN_bn2bin_padded(r_secret_x.data(), size, rx));
  REQUIRE(BN_bn2bin_padded(r_secret_y.data(), size, ry));

  std::unique_ptr<TLSSocket> server, client;
  REQUIRE(setup_sockets(context, server, client));

  size_t size_of_p_v, size_of_p_p;
  BIGNUM *sb = BN_CTX_get(sbn_ctx.get()), *rb = BN_CTX_get(rbn_ctx.get());
  uint64_t prover_rounds = 0, prover_bw = 0;

  auto verifier_code = [&]() {
    REQUIRE(ECtF::ectf_produce_bignum(sb, server->get_ssl_object(), s_secret_x,
                                      s_secret_y, curve_id, true, &size_of_p_v));
  };
  std::thread verifier(verifier_code);
  REQUIRE(ECtF::ectf_produce_bignum(rb, client->get_ssl_object(), r_secret_x,
                                    r_secret_y, curve_id, false, &size_of_p_p,
                                    /*track_bandwidth=*/true, &prover_bw,
                                    &prover_rounds));
  verifier.join();

  MESSAGE("prover ECtF rounds (batched) = " << prover_rounds);
  // The unbatched implementation used 11 prover rounds; batching must reduce it.
  CHECK(prover_rounds < 11);
}
//! [ECtFBatchedRounds]
