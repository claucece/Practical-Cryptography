...
/**
     set_derive_resumption_secrets_callback. This function sets the
  derive_resumption_secret callback function for the underlying ssl object to
  `cb`. If `cb` is not a null pointer, then the function `cb` will be called
  after `this` shocket has generated the shared x co-ordinate for an SSL
  connection with another node. This function returns true in case of success
  and false otherwise. This function will return false if:
  ...
  responsible for checking this prior to a dereference.
  **/
  bool set_derive_resumption_keys_callback(
      ThreePartyHandshake::derive_resumption_keys_function_type cb =
          &ThreePartyHandshake::derive_resumption_keys) noexcept;

/**
     set_derive_psk_secrets_callback. This function sets the
  derive_psk_secret callback function for the underlying ssl object to
  `cb`. If `cb` is not a null pointer, then the function `cb` will be called
  after `this` shocket has generated the shared x co-ordinate for an SSL
  connection with another node. This function returns true in case of success
  and false otherwise. This function will return false if:
  ...
  responsible for checking this prior to a dereference.
  **/
  bool set_derive_psk_keys_callback(
      ThreePartyHandshake::derive_psk_keys_function_type cb =
          &ThreePartyHandshake::derive_psk_keys) noexcept;

/**
     set_commit_to_session_ticket_callback. This function sets the
  set_commit_to_session_ticket callback function for the underlying ssl object to
  `cb`. If `cb` is not a null pointer, then the function `cb` will be called
  after `this` shocket has generated the shared x co-ordinate for an SSL
  connection with another node. This function returns true in case of success
  and false otherwise. This function will return false if:
  ...
  responsible for checking this prior to a dereference.
  **/
  bool set_commit_to_session_ticket_callback(
      ThreePartyHandshake::commit_to_session_ticket_function_type cb =
          &ThreePartyHandshake::commit_to_session_ticket) noexcept;
  /**
  ...
  **/
  ...
  bool set_combine_handshake_secret_shares_callback(
      ThreePartyHandshake::combine_handshake_secret_shares_function_type cb =
          &ThreePartyHandshake::combine_handshake_secret_shares) noexcept;
  ...
  **/
  ...
  /**
     set_surf_encrypt_callback. Sets the surf_encrypt callback on the underlying
  ssl object. Called from tls_seal_record once the 3P traffic keys are
  installed: on the MPC path the client traffic key is secret-shared, so a
  record cannot be sealed locally and must be produced jointly with the verifier
  (Algorithm 1). Returns false if this is a server socket. Does not throw.

     If this is never called, the hook silently does not fire and records are
  sealed with the local AEAD context instead, which on the prover holds only a
  mask rather than the real key.

     @param[in] cb: the callback function.
     @return true if setting is successful, false otherwise.
  **/
  bool set_surf_encrypt_callback(
      ThreePartyHandshake::surf_encrypt_function_type cb =
          &ThreePartyHandshake::encrypt_request) noexcept;

  /**
     set_surf_decrypt_callback. Sets the surf_decrypt callback on the underlying
  ssl object. Called from tls_open_record right after each application record is
  captured: verifies that record's GCM tag and derives its keystream, so
  attestation runs inline per record rather than as one batch at shutdown.
  Returns false if this is a server socket. Does not throw.

     If this is never called, the hook silently does not fire and records
  accumulate in ssl->surf_records unattested.

     @param[in] cb: the callback function.
     @return true if setting is successful, false otherwise.
  **/
  bool set_surf_decrypt_callback(
      ThreePartyHandshake::surf_decrypt_function_type cb =
          &ThreePartyHandshake::attest_record) noexcept;

  /**
     set_rotate_traffic_keys_callback. ROTATE mode: called from
  tls13_rotate_traffic_key on SSL_key_update and on a received KeyUpdate so the
  rotation runs as a joint circuit. Returns false if this is a server socket.
  **/
  bool set_rotate_traffic_keys_callback(
      ThreePartyHandshake::rotate_traffic_keys_function_type cb =
          &ThreePartyHandshake::rotate_traffic_keys) noexcept;

  /**
     set_derive_binder_callback. Sets the derive_binder callback on the
  underlying ssl object. Called from tls13_write_psk_binder during construction
  of a resumed ClientHello: the PSK is secret-shared, so the binder must be
  computed jointly with the verifier rather than locally. Returns false if this
  is a server socket. Does not throw.

     If this is never called, the hook silently does not fire and the origin
  rejects the resumed ClientHello with a decrypt_error alert.

     @param[in] cb: the callback function.
     @return true if setting is successful, false otherwise.
  **/
  bool set_derive_binder_callback(
      ThreePartyHandshake::derive_binder_function_type cb =
          &ThreePartyHandshake::derive_binder) noexcept;
  ...
  **/
  ...
  /**
     preprocess_circuits. Runs the offline garbled-circuit preprocessing phase
  against this connection's verifier. Provers call this *before* connect_to() so
  the slow MPC preprocessing happens before the origin connection is opened,
  avoiding the origin's idle timeout over a WAN link. Idempotent and a no-op when
  circuits are disabled (tests) or already built. Returns false on error. Does
  not throw.
  **/
  bool preprocess_circuits() noexcept;

  /**
     set_session. Configures |this| SSL connection to offer to resume |session|
  in the next handshake. Must be called before connect_to(). The caller retains
  ownership of |session|. Returns false if |session| is null or this is a
  server socket. Does not throw.
     @param[in] session: a non-owning pointer to the session to resume.
     @return true in case of success, false otherwise.
  **/
  bool set_session(SSL_SESSION *session) noexcept;

  /**
     set_accept_timeout. Sets a timeout on accept(). Delegates to
  StatefulSocket::set_accept_timeout. Must be called after listen() (i.e. after
  the Server constructor completes). Returns false if setsockopt fails.
