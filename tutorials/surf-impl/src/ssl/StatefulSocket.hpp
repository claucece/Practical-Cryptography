...
public:
  ...
  **/
  ...
  /**
     set_accept_timeout. This function sets a timeout on accept(). After
  |milliseconds| of blocking without a new connection, accept() returns false
  (errno == EAGAIN). Must be called after listen() (i.e. after the Server
  constructor completes). Returns false if the socket is not listening or
  setsockopt fails.
