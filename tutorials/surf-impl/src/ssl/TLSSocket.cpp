...
bool TLSSocket::set_verifier_connection(SSL *const verifier) noexcept {
  if (is_server()) {
    std::cerr << "Cannot set verifier connection on a server socket." << std::endl;
    return false;
  }
...
bool TLSSocket::connect_to(const std::string &address, const uint16_t port_number) {
...
int TLSSocket::do_handshake() noexcept {
  ...
  if (!is_connection_valid()) {
    ...
    std::cerr << "Connection not valid. Please call accept first." << std::endl;
...
bool TLSSocket::set_derive_resumption_keys_callback(
    ThreePartyHandshake::derive_resumption_keys_function_type cb) noexcept {
  // This check needs some explaining.
  // This function can be called at any time -- even before do_handshake or
  // accept. As a result, even if this socket has a type, it isn't guaranteed
  // that `ssl` knows this fact. This means that the call to
  ...
  return SSL::set_derive_resumption_keys(ssl, cb);
}

bool TLSSocket::set_derive_psk_keys_callback(
    ThreePartyHandshake::derive_psk_keys_function_type cb) noexcept {
  // This check needs some explaining.
  // This function can be called at any time -- even before do_handshake or
  // accept. As a result, even if this socket has a type, it isn't guaranteed
  // that `ssl` knows this fact. This means that the call to
  ...
  return SSL::set_derive_psk_keys(ssl, cb);
}

bool TLSSocket::set_commit_to_session_ticket_callback(
    ThreePartyHandshake::commit_to_session_ticket_function_type
        cb) noexcept {
  if (is_server()) {
    return false;
  }
  return SSL::set_commit_to_session_ticket(ssl, cb);
}
...
bool TLSSocket::set_combine_handshake_secret_shares_callback(
    ThreePartyHandshake::combine_handshake_secret_shares_function_type
        cb) noexcept {
  if (is_server()) {
    return false;
  }
  return SSL::set_combine_handshake_secret_shares(ssl, cb);
}
...
bool TLSSocket::set_surf_encrypt_callback(
    ThreePartyHandshake::surf_encrypt_function_type cb) noexcept {
  // Same reasoning as the other setters: this can be called before accept() or
  // do_handshake(), so `ssl` may not yet know which side it is. We check here
  // rather than relying on SSL::set_* to fail.
  if (is_server()) {
    return false;
  }

  return SSL::set_surf_encrypt(ssl, cb);
}

bool TLSSocket::set_surf_decrypt_callback(
    ThreePartyHandshake::surf_decrypt_function_type cb) noexcept {
  if (is_server()) {
    return false;
  }

  return SSL::set_surf_decrypt(ssl, cb);
}

bool TLSSocket::set_rotate_traffic_keys_callback(
    ThreePartyHandshake::rotate_traffic_keys_function_type cb) noexcept {
  if (is_server()) {
    return false;
  }
  return SSL::set_rotate_traffic_keys(ssl, cb);
}

bool TLSSocket::set_derive_binder_callback(
    ThreePartyHandshake::derive_binder_function_type cb) noexcept {
  if (is_server()) {
    return false;
  }
  return SSL::set_derive_binder(ssl, cb);
}
...
bool TLSSocket::preprocess_circuits() noexcept {
  if (!ssl) {
    return false;
  }
  return ThreePartyHandshake::preprocess_circuits(ssl);
}

bool TLSSocket::set_session(SSL_SESSION *session) noexcept {
  if (!session || is_server()) return false;
  return SSL_set_session(ssl, session) == 1;
}

bool TLSSocket::set_accept_timeout(int milliseconds) noexcept {
  return StatefulSocket::set_accept_timeout(milliseconds);
}
