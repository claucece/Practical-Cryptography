...
EmpWrapperAG2PC::EmpWrapperAG2PC(SSL *const ssl,
    ...
    : wrapper{ssl}, addr{&wrapper, &wrapper},
      // Resolve the circuit path so it loads regardless of the process's cwd
      // (the binaries live in build/, but may be launched from elsewhere).
      // NOTE: get_secret_size below must keep the ORIGINAL `filepath` pointer:
      // it deliberately relies on pointer identity with the FILEPATH constants.
      circuit{Util::resolve_circuit_path(filepath).c_str()}, twopc{nullptr},
...
bool EmpWrapperAG2PC::exec_small_internal(
  ...
  // If the connection has already failed (or a consistency check tripped),
  // anything we'd compute here is garbage — fail the run immediately.
  if (wrapper.has_io_failed()) {
    return false;
  }
  /*
  ...
  */
  ...
  const auto mode = [&]() -> int {
    ...
    case CircuitType::KS_BATCH:
      return ks_batch->party;
    case CircuitType::AES_ENC:
      return aes_enc->party;
    case CircuitType::ROTATE:
      return rotate->party;
    default:
      assert(false);
  ...
  // The whole online phase runs under a catch: emp's malicious-security
  // checks (e.g. "cheat CT!" in helper.h) throw in our vendored copy rather
  // than exit(1)-ing the process, and allocation can fail on huge (garbage)
  // sizes after a desync. Either way the run is dead, not the process.
  const auto t_online = std::chrono::steady_clock::now();
  try {
    switch (type) {
    case CircuitType::SINGLE:
    ...
    case CircuitType::GCM_VFY:
      ...
      break;
    case CircuitType::KS_BATCH:
      ks_batch->online(as_bool.data(), temp_out.data());
      break;
    case CircuitType::AES_ENC:
      aes_enc->online(as_bool.data(), temp_out.data());
      break;
    case CircuitType::ROTATE:
      rotate->online(as_bool.data(), temp_out.data());
      break;
    default:
      assert(false);
      __builtin_unreachable();
    ...
    std::cerr << "[EmpWrapperAG2PC] online failed: " << e.what() << "\n";
    wrapper.mark_io_failed();
    return false;
  }
  const auto online_ms = std::chrono::duration<double, std::milli>(
      std::chrono::steady_clock::now() - t_online).count();

  // The online phase may have died mid-circuit (peer gone): the IO layer
  // cannot report that to emp, so check it here before trusting the output.
  if (wrapper.has_io_failed()) {
    return false;
  }
...
bool EmpWrapperAG2PC::verify_tag_commit(
...
bool EmpWrapperAG2PC::derive_ks_batch(
    const EmpWrapperAG2PC::aes_ks_batch_input_type &in,
    EmpWrapperAG2PC::aes_ks_batch_output_type &out) noexcept {
  if (run_times == ks_iters) {
    do_preproc();
  }
  return exec_small_internal(in, out);
}

bool EmpWrapperAG2PC::encrypt_batch(
    const EmpWrapperAG2PC::aes_enc_batch_input_type &in,
    EmpWrapperAG2PC::aes_enc_batch_output_type &out) noexcept {
  if (run_times == aes_enc_iters) {
    do_preproc();
  }
  return exec_small_internal(in, out);
}
...
bool EmpWrapperAG2PC::derive_ts(
  ...
  if (run_times > 0) {
    std::cerr << "[ag2pc] derive_ts RE-PREPROCESSING\n";
    do_preproc();
  }
  return exec_small_internal(in, out);
}

bool EmpWrapperAG2PC::derive_rotate(
    const EmpWrapperAG2PC::derive_rotate_input_type &in,
    EmpWrapperAG2PC::derive_rotate_output_type &out) noexcept {
  if (run_times == rotate_iters) {
    do_preproc();
  }
  return exec_small_internal(in, out);
}

bool EmpWrapperAG2PC::derive_res(
    const EmpWrapperAG2PC::derive_res_input_type &in,
    EmpWrapperAG2PC::derive_res_output_type &out) noexcept {
  if (run_times > 0) {
    do_preproc();
  }
  return exec_small_internal(in, out);
...
bool EmpWrapperAG2PC::derive_psk(
    const EmpWrapperAG2PC::derive_psk_input_type &in,
    EmpWrapperAG2PC::derive_psk_output_type &out) noexcept {
  if (run_times > 0) {
    do_preproc();
  }
  return exec_small_internal(in, out);
...
bool EmpWrapperAG2PC::derive_psk(
    const EmpWrapperAG2PC::derive_psk_input_type_8 &in,
    EmpWrapperAG2PC::derive_psk_output_type &out) noexcept {
  if (run_times > 0) {
    do_preproc();
  }
...
bool EmpWrapperAG2PC::derive_psk(
    const EmpWrapperAG2PC::derive_psk_input_type_0 &in,
    EmpWrapperAG2PC::derive_psk_output_type &out) noexcept {
  if (run_times > 0) {
    do_preproc();
  }
...
bool EmpWrapperAG2PC::derive_binder(const derive_binder_input_type &in,
...
void EmpWrapperAG2PC::set_gcm_vfy_commit(const int mode,
...
void EmpWrapperAG2PC::set_ks_batch(const int mode, const int tag) noexcept {
  assert(mode == emp::ALICE || mode == emp::BOB);
  assert(type == CircuitType::NONE);
  ks_batch.reset(new KSBlockCircuitType(addr, mode, tag, &circuit));
  type = CircuitType::KS_BATCH;
}

void EmpWrapperAG2PC::set_aes_enc(const int mode, const int tag) noexcept {
  assert(mode == emp::ALICE || mode == emp::BOB);
  assert(type == CircuitType::NONE);
  aes_enc.reset(new AESEncCircuitType(addr, mode, tag, &circuit));
  type = CircuitType::AES_ENC;
}

void EmpWrapperAG2PC::set_rotate(const int mode, const int tag) noexcept {
  assert(mode == emp::ALICE || mode == emp::BOB);
  assert(type == CircuitType::NONE);
  rotate.reset(new RotateCircuitType(addr, mode, tag, &circuit));
  type = CircuitType::ROTATE;
}
...
EmpWrapperAG2PC *
EmpWrapperAG2PC::build_derive_rotate_circuit(SSL *const ssl, const int mode,
                                             const int tag) noexcept {
  auto res = new EmpWrapperAG2PC(
      ssl, EmpWrapperAG2PCConstants::ROTATE_KEY_FILEPATH);
  res->set_rotate(mode, tag);
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_derive_res_circuit(SSL *const ssl, const int mode,
                                         const int tag) noexcept {
  auto res = new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::RMS_FILEPATH);
  res->set_twopc(mode, tag);
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_derive_psk_circuit(SSL *const ssl, const int mode,
                                          const int tag) noexcept {
  auto res = new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::PSK_FILEPATH);
  res->set_twopc(mode, tag);
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_derive_psk_circuit_8(SSL *const ssl, const int mode,
                                            const int tag) noexcept {
  auto res = new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::PSK_FILEPATH_8);
  res->set_twopc(mode, tag);
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_derive_psk_circuit_0(SSL *const ssl, const int mode,
                                            const int tag) noexcept {
  auto res = new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::PSK_FILEPATH_0);
  res->set_twopc(mode, tag);
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_derive_binder_circuit(SSL *const ssl, const int mode,
                                             const int tag) noexcept {
  auto *const ptr =
      new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::BINDER_FILEPATH);
  if (!ptr) {
    return nullptr;
  }
  ptr->set_twopc(mode, tag);
  return ptr;
}
...
EmpWrapperAG2PC *
EmpWrapperAG2PC::build_gcm_vfy_commit_circuit(SSL *const ssl, const int mode,
                                              const int tag) noexcept {
  auto res = new EmpWrapperAG2PC(
      ssl, EmpWrapperAG2PCConstants::AES_GCM_VFY_COMMIT_FILEPATH);
  res->set_gcm_vfy_commit(mode, tag);
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_ks_batch_circuit(SSL *const ssl, const int mode,
                                        const int tag) noexcept {
  auto res =
      new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::KS_BATCH_FILEPATH);
  res->set_ks_batch(mode, tag);
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_aes_enc_circuit(SSL *const ssl, const int mode,
                                       const int tag) noexcept {
  auto res =
      new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::AES_ENC_FILEPATH);
  res->set_aes_enc(mode, tag);
  return res;
}
...
void EmpWrapperAG2PC::do_preproc_indep() noexcept {
  // Skip if the connection has already failed: emp cannot be told about IO
  // errors, so running preprocessing over a dead connection just burns CPU on
  // garbage. The failure is reported to callers via has_io_failed().
  if (wrapper.has_io_failed()) {
    return;
  }
  // emp's malicious-security checks (e.g. FEQ in fpre) throw in our vendored
  // copy rather than exit(1)-ing the process; treat that like a dead
  // connection.
  try {
    switch (type) {
    case CircuitType::SINGLE:
    ...
    case CircuitType::GCM_TAG:
      ...
      return;
    case CircuitType::KS_BATCH:
      ks_batch->function_independent();
      return;
    case CircuitType::AES_ENC:
      aes_enc->function_independent();
      return;
    case CircuitType::ROTATE:
      rotate->function_independent();
      return;
    default:
      assert(false);
    }
  } catch (const std::exception &e) {
    std::cerr << "[EmpWrapperAG2PC] preproc (indep) failed: " << e.what()
              << "\n";
    wrapper.mark_io_failed();
  }
}
...
void EmpWrapperAG2PC::do_preproc_dep() noexcept {
  run_times = 0;
  // See do_preproc_indep for why we guard and catch here.
  if (wrapper.has_io_failed()) {
    return;
  }
  try {
    ...
    case CircuitType::GCM_TAG:
      ...
      return;
    case CircuitType::KS_BATCH:
      ks_batch->function_dependent();
      return;
    case CircuitType::AES_ENC:
      aes_enc->function_dependent();
      return;
    case CircuitType::ROTATE:
      rotate->function_dependent();
      return;
    default:
      assert(false);
    }
  } catch (const std::exception &e) {
    std::cerr << "[EmpWrapperAG2PC] preproc (dep) failed: " << e.what()
              << "\n";
    wrapper.mark_io_failed();
  }
}
