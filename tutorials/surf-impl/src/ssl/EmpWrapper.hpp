...
#include <cstdio> // for the temporary EmpWrapper framing diagnostics
...
public:
  ...
  **/
  ...
  /**
     get_round_counter. This function returns the number of outbound messages
  flushed via this socket if tracking has been enabled. This is a proxy for the
  number of communication rounds (one count per message put on the wire). This
  function does not throw.
     @return the number of rounds (outbound messages) for this socket.
  **/
  inline std::size_t get_round_counter() const noexcept;

  /**
     reset_round_counter. This function resets the round counter to 0 if
  tracking has been enabled. This function does not throw.
  **/
  inline void reset_round_counter() noexcept;
  ...
  **/
  ...
  /**
     has_io_failed. This function returns true if a fatal I/O error (peer
  reset, clean shutdown, ...) has occurred on this wrapper. emp's IOChannel
  interface cannot report errors, so once this is set all further sends and
  receives on this wrapper are silent no-ops; whoever drives a 2PC run (e.g.
  EmpWrapperAG2PC) must check this and abandon the run, since any data
  "transferred" after the failure is garbage. This function does not throw.
     @return true if a fatal I/O error has occurred on this wrapper.
  **/
  inline bool has_io_failed() const noexcept;

  /**
     mark_io_failed. This function poisons the wrapper as if a fatal I/O error
  had occurred (see has_io_failed()). Used when a failure is detected outside
  the I/O layer — e.g. a failed 2PC consistency check — after which the byte
  stream can no longer be trusted. This function does not throw.
  **/
  inline void mark_io_failed() noexcept;
...
private:
  ...
  **/
  ...
  /**
     io_failed. Latched to true on the first fatal I/O error; see
  has_io_failed().
  **/
