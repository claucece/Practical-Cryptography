...
static FILE* g_keylog = nullptr;

static void KeyLogCallback(const SSL*, const char* line) {
  if (!g_keylog) {
    const char* path = std::getenv("SSLKEYLOGFILE");
    if (!path) path = "/tmp/keys.log";   // fallback if env not set
    g_keylog = std::fopen(path, "a");
    if (!g_keylog) return;
  }
  std::fprintf(g_keylog, "%s\n", line);
  std::fflush(g_keylog);
}
...
TEST_CASE("TLSSocketConnectToTests") {
  ...
  // TLSSockets to either send or receive data.
  auto ctx = CreateContextWithTestCertificate(TLS_method());
  REQUIRE(ctx);
  SSL_CTX_set_keylog_callback(ctx.get(), KeyLogCallback);
  TLSSocket server(*ctx), client(*ctx, false);
  ...
  SUBCASE("Connection setup works") {
    SUBCASE("IPV4 connection setup works") {
      ...
      REQUIRE(SSL_is_init_finished(client.get_ssl_object()));
      CHECK(SSL_get_current_cipher(client.get_ssl_object()) != nullptr);
      REQUIRE(std::string(SSL_get_version(client.get_ssl_object())) == "TLSv1.3");
      REQUIRE(std::string(SSL_get_version(server.get_ssl_object())) == "TLSv1.3");

      bool ok = res.get();
      CHECK(ok);
      std::cerr << "LOL " << std::boolalpha << ok << '\n';

      // Most importantly: we care that the SSL object has a non-null BIO.
      CHECK(client.get_ssl_object()->rbio);
      CHECK(client.get_ssl_object()->wbio);
      CHECK(server.get_ssl_object()->rbio);
      CHECK(server.get_ssl_object()->wbio);
    }
  ...
  // reading and writing.
  static const char TestMessage[] = "TestMessage"; // which size is 12
  ...
  int wret = -1;
  REQUIRE(client.write(TestMessage, sizeof(TestMessage), &wret));
  CHECK(wret == sizeof(TestMessage));

  //char response[sizeof(TestMessage)];
  //REQUIRE(client.read(response, sizeof(response)) == sizeof(TestMessage));

      CHECK(client.get_ssl_object()->wbio);
//int r = client.read(response, sizeof(response));
//if (r != sizeof(TestMessage)) {
//    int err = client.get_ssl_error(r);
//    CHECK(client.get_ssl_error(r) == SSL_ERROR_SSL);

//    FAIL("SSL_read failed: return=" << r << " ssl_error=" << err);
//}
