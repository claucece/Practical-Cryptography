...
#include <cassert>
#include <cerrno>  // Needed for errno in I/O failure diagnostics.
#include <climits>
#include <cstdint>
#include <cstdio>  // Needed for fprintf in I/O failure diagnostics.
#include <cstring> // Needed for strerror in I/O failure diagnostics.
#include <functional>
#include <poll.h> // Needed to wait out SSL_ERROR_WANT_READ/WANT_WRITE in process_data.
#include <string>
...
/**
   executable_dir. Returns the absolute directory containing the currently
   running executable, or an empty string if it cannot be determined. Useful
   for locating data files bundled alongside the build (e.g. the Bristol-format
   circuits) independently of the process's current working directory. This
   function does not throw.
   @return the absolute path to the executable's directory, or "" on failure.
**/
std::string executable_dir() noexcept;

/**
   resolve_circuit_path. Resolves a (possibly relative) circuit file path to one
   that actually exists, so circuits load regardless of the working directory
   the binary was launched from. Resolution order:
     1. if $SURF_CIRCUIT_ROOT is set and $SURF_CIRCUIT_ROOT/<filepath> exists,
        use that;
     2. if <filepath> exists relative to the current working directory, use it
        unchanged (preserves existing behaviour when run from build/);
     3. otherwise try <executable_dir>/<filepath>.
   Absolute paths are returned unchanged. If nothing matches, the original
   filepath is returned so the caller's own error handling still runs. This
   function does not throw.
   @param[in] filepath: the circuit path. Must not be null: asserted in debug.
   @return a resolved path string (see resolution order above).
**/
std::string resolve_circuit_path(const char *const filepath) noexcept;
...
SSL3_RT_MAX_PLAIN_LENGTH in either direction. This function does not throw.
   Transient SSL_ERROR_WANT_READ/WANT_WRITE results are waited out and retried;
   any other failure is logged and abandons the operation.
   ...
   @return true if all `nbyte` bytes were processed, false on a fatal I/O
   error (the buffer may be part-filled; the connection should be treated as
   dead).
**/
template <typename RT, typename F>
bool process_data(SSL *const in, RT *const data, const std::size_t nbyte,
