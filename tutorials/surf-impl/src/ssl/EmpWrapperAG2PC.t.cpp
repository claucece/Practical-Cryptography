...
template <typename T, int mode>
static T *build_res_circuit(SSL *const ssl) noexcept {
  return T::build_derive_res_circuit(ssl, mode, 1);
}
...
produce_sublabel(const std::string &label,
  ...
  if (label == "finished" || label == "iv" || label == "key" || label == "iv" || label == "res master") {
    tmp_hash = nullptr;
    tmp_hash_size = 0;

    if (label == "key") {
      hash_output_size = 16;
    } else if (label == "finished" || label == "res master") {
      hash_output_size = 32;
    } else {
      hash_output_size = 12;
    }
...
TEST_CASE_TEMPLATE(
  ...
  constexpr auto out_size =
      EmpWrapperAG2PCConstants::HANDSHAKE_SECRETS_OUTPUT_SIZE; // 220
  ...
  SUBCASE("empty params") {
    ...
    SUBCASE("should be deterministic") {
  ...
  SUBCASE("is_correct") {
    ...
    // Choose a random hash value: this does not matter as we don't store for it.
    std::array<uint8_t, 32> h2{};
    for (unsigned i = 0; i < 32; i++) {
      h2[i] = static_cast<uint8_t>(rand());
    }

    // Now we start packing. Alice feeds dES first (32 bytes), then h2 (32
    // bytes), then mask, then the additive DHE share. This matches the
    // Alice-input wire order in ProduceCombinedCircuit.
    std::copy(dES.cbegin(), dES.cend(), client_in.begin());
    std::copy(h2.cbegin(), h2.cend(), client_in.begin() + 32);
    ...
    for (unsigned i = 0; i < client_mask.size(); i++) {
      client_in[64 + i] = client_mask[i] = static_cast<uint8_t>(rand());
      server_in[64 + i] = server_mask[i] = static_cast<uint8_t>(rand());
    }
    ...
    constexpr auto offset =
        32 + 32 + EmpWrapperAG2PCConstants::HANDSHAKE_MASK_SIZE;
    ...
    // xor'd against the server's mask. 5 comes from the number of secrets
    unsigned pos{};
