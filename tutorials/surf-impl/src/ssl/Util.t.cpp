...
TEST_CASE("InitialiseCBBForEcPointData") {
  ...
  const uint16_t group_id = SSL_CURVE_SECP256R1;
...
TEST_CASE("RandomECPoint") {
  ...
  SUBCASE("RandomECPoint fails on nullptr input") {
    ...
    uint16_t curve = SSL_CURVE_SECP256R1;
  ...
  SUBCASE("Randomly generated points live on a curve") {
    ...
    std::array<uint16_t, 4> curves{SSL_CURVE_SECP256R1, SSL_CURVE_SECP256R1,
...
TEST_CASE("ComputePremasterSecret") {
  auto prover = bssl::SSLKeyShare::Create(SSL_CURVE_SECP256R1);
  auto verifier = bssl::SSLKeyShare::Create(SSL_CURVE_SECP256R1);
  auto server = bssl::SSLKeyShare::Create(SSL_CURVE_SECP256R1);
  REQUIRE(prover);
  REQUIRE(verifier);
  REQUIRE(server);
...
TEST_CASE("get_hash") {
  ...
  bssl::SSLTranscript transcript{false};
