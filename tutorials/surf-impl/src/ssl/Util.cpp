...
bool Util::CBBToECPoint(EC_GROUP *const group,
  ...
  auto point_span = bssl::Span(CBS_data(&out_cbs), CBS_len(&out_cbs));
...
bool Util::generate_public_key(bssl::SSLKeyShare &client,
  ...
  if (!client.Generate(&key_exchange)) {
    return false;
  }
...
static bool compute_premaster_secret_impl(
  ...
  if (!Util::deinitialise_cbb_for_ec_point_data(&group_id, &in_cbs,
      ...
      !client.Decap(&out_x, &alert, peer_key, out_y)) {
    return false;
  }
...
std::string Util::executable_dir() noexcept {
  std::string path;
#if defined(__APPLE__)
  // _NSGetExecutablePath: first call with a null buffer to learn the required
  // size, then fetch the path.
  uint32_t size = 0;
  ...
  path.assign(buf.c_str()); // trim at the embedded NUL
#elif defined(__linux__)
  char buf[PATH_MAX] = {};
  ...
  // Canonicalise (resolve symlinks / "..") so the result is a clean absolute
  // path; fall back to the raw path if realpath fails.
  char real[PATH_MAX] = {};
...
std::string Util::resolve_circuit_path(const char *const filepath) noexcept {
  assert(filepath);
  ...
  // 1) Explicit override via $SURF_CIRCUIT_ROOT.
  if (const char *const root = std::getenv("SURF_CIRCUIT_ROOT")) {
    std::string candidate = std::string(root) + "/" + filepath;
    if (is_valid_filepath(candidate.c_str())) {
      return candidate;
    }
  }

  // 2) Relative to the current working directory (back-compat: this is what
  //    already worked when the binary was launched from the build dir).
  if (is_valid_filepath(filepath)) {
    return std::string(filepath);
  }

  // 3) Relative to the executable's own directory. The binaries live in the
  //    build dir, so "../2pc/..." resolves correctly regardless of cwd.
  const std::string dir = executable_dir();
  if (!dir.empty()) {
    std::string candidate = dir + "/" + filepath;
    if (is_valid_filepath(candidate.c_str())) {
      return candidate;
    }
  }

  // Nothing matched; hand back the original so the caller's error path runs.
  return std::string(filepath);
}
