...
TEST_CASE("ThreePartyHandshakeIdealFunctionality") {
  ...
  // Note: we'll iterate this process over the supported NIST curves
  std::array<uint16_t, 3> curves{SSL_CURVE_SECP256R1,
...
//! [ThreePartyDeriveResumptionKeysTests]
TEST_CASE("ThreePartyHandshakeDeriveResumptionKeys and SSL interop") {
  bssl::UniquePtr<SSL_CTX> context(SSL_CTX_new(TLS_method()));
  bssl::UniquePtr<SSL> ssl(SSL_new(context.get()));
  bssl::SSL_HANDSHAKE hs{ssl.get()};
  ...
  SUBCASE("DRK does sensible things") {
    SUBCASE("DRK returns false on nullptr") {
      CHECK(!ThreePartyHandshake::derive_resumption_keys(nullptr, nullptr));
      CHECK(!ThreePartyHandshake::derive_resumption_keys(nullptr, ssl.get()));
      CHECK(!ThreePartyHandshake::derive_resumption_keys(&hs, nullptr));
    }

    SUBCASE("DRK returns false if verifier is not set") {
      CHECK(!ThreePartyHandshake::derive_resumption_keys(&hs, ssl.get()));
    }

    SUBCASE("DRK returns false if verifier is a server") {
      SSL_set_accept_state(ssl.get());
      ssl->verifier = ssl.get();
      CHECK(!ThreePartyHandshake::derive_resumption_keys(&hs, ssl.get()));
    }
...
//! [ThreePartyDerivePskKeysTests]
TEST_CASE("ThreePartyHandshakeDerivePskKeys and SSL interop") {
  bssl::UniquePtr<SSL_CTX> context(SSL_CTX_new(TLS_method()));
  bssl::UniquePtr<SSL> ssl(SSL_new(context.get()));
  ...
  SUBCASE("DPK does sensible things") {
    SUBCASE("DPK returns false on nullptr") {
      CHECK(!ThreePartyHandshake::derive_psk_keys(nullptr));
    }

    SUBCASE("DPK returns false if verifier is not set") {
      CHECK(!ThreePartyHandshake::derive_psk_keys(ssl.get()));
    }

    SUBCASE("DPK returns false if verifier is a server") {
      SSL_set_accept_state(ssl.get());
      ssl->verifier = ssl.get();
      CHECK(!ThreePartyHandshake::derive_psk_keys(ssl.get()));
    }
