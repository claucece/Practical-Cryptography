...
TEST_CASE("Building a server works") {
  std::cerr << "Testing server construction..." << std::endl;
...
TEST_CASE("Creating a new share works") {
  std::cerr << "Testing creating a new share..." << std::endl;
...
TEST_CASE("Getting the CurveID works") {
  std::cerr << "Testing getting the curve ID..." << std::endl;
  Server server(CreateContextWithTestCertificate(TLS_method()), "127.0.0.1",
                false, 1);
...
TEST_CASE("We can make multiple shares") {
  std::cerr << "Testing creating multiple shares..." << std::endl;
  Server server(CreateContextWithTestCertificate(TLS_method()), "127.0.0.1",
                false, 1);
  ...
  SUBCASE("Otherwise it's as expected") {
    ...
    // We'll try it for all curves.
    constexpr std::array<uint16_t, 3> curves{
        SSL_CURVE_SECP256R1, SSL_CURVE_SECP384R1,
        SSL_CURVE_SECP521R1};
    ...
    for (unsigned i = 0; i < curves.size(); i++) {
      ...
      for (unsigned j = 0; j < curves.size(); j++) {
        ...
        REQUIRE(key_shares[0]->Generate(&key_exchange));

        REQUIRE(CBB_add_u16(cbb.get(), key_shares[1]->GroupID()));
        REQUIRE(CBB_add_u16_length_prefixed(cbb.get(), &key_exchange));
        REQUIRE(key_shares[1]->Generate(&key_exchange));
...
TEST_CASE("Server get_additive_share tests") {
  std::cerr << "Testing getting the additive share..." << std::endl;
  Server server(CreateContextWithTestCertificate(TLS_method()), "127.0.0.1",
                false, 1);
  ...
  SUBCASE(
    ...
    constexpr auto curve = SSL_CURVE_SECP256R1;
    bssl::UniquePtr<EC_GROUP> group(
        EC_GROUP_new_by_curve_name(Util::get_nid_from_uint16(curve)));
    REQUIRE(group);
    auto other_party = bssl::SSLKeyShare::Create(curve);
...
TEST_CASE("Server write_handshake_done tests") {
  std::cerr << "Testing writing handshake done..." << std::endl;
...
TEST_CASE("Server send_additive_share tests") {
  std::cerr << "Testing sending the additive share..." << std::endl;
...
TEST_CASE("The three party handshake works") {
  std::cerr << "Testing the three party handshake..." << std::endl;
...
//! [ServerRunTests]
TEST_CASE("Run tests") {
  std::cerr << "Testing running the server..." << std::endl;
  Server verifier(CreateContextWithTestCertificate(TLS_method()), "127.0.0.1",
                  false, 1);
  Server server(CreateContextWithTestCertificate(TLS_method()), "127.0.0.1",
                false, 1);
...
TEST_CASE("ServerRunTestsSendKS") {
  std::cerr << "Testing running the server with sending keyshares..." << std::endl;
  ...
  std::thread server_thread(
    [server, prom = std::move(server_promise)]() mutable {
      server_hs(std::move(prom), server.get());
    }
  );
...
TEST_CASE("secret_derivation") {
  std::cerr << "Testing that the secrets derived from the 3PH are correct..."
            << std::endl;
  ...
  Server verifier(CreateContextWithTestCertificate(TLS_method()), "127.0.0.1",
                  false, 1);
  auto server = std::make_shared<Server>(
    CreateContextWithTestCertificate(TLS_method()), "127.0.0.1", false, 1);
