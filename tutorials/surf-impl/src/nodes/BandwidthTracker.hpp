...
public:
  ...
  // Per-event byte counts recorded since the last reset. Used by benchmarks to
  // emit per-step bandwidth columns, mirroring Timer::get_times().
  const DataArray &get_data() const noexcept { return data; }

  // Sum of all per-event byte counts.
