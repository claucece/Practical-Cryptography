...
bool KeyShare::finish(CBS &in) noexcept {
  uint8_t alert;
  return (this->share &&
          this->share->Decap(&out_secret, &alert, in, &out_other_secret));
}
...
bool KeyShare::create_new_public_key(const uint16_t curve_id) noexcept {
  ...
  if (curve_id == SSL_CURVE_X25519 || curve_id == SSL_GROUP_X25519_MLKEM768 || curve_id == SSL_GROUP_X25519_KYBER768_DRAFT00) {
    return false;
  }
