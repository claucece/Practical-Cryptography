...
static void execute_bench_ot(CircuitType &circ, T &io, const int mode,
  ...
  assert(circuit_file.n1 == CHAR_BIT * circ.input_bytes);
  assert(circuit_file.n2 == CHAR_BIT * circ.input_bytes);
  assert(circuit_file.n3 == CHAR_BIT * circ.output_bytes);
  ...
  if (alice) {
    ...
    for (unsigned i = 1; i < iterations; i++) {
      ...
      // NOTE: setup/indep_preproc/dep_preproc/exec are stored as
      // duration<double, std::milli>, so .count() yields fractional
      // milliseconds. Do NOT duration_cast to integer milliseconds here: the
      // garbled-circuit online phase for the AES circuits runs in under 1 ms
      // and would otherwise truncate to 0.
    ...
    std::cout << "setup_avg (ms):" << (setup_avg / iterations).count() << '\n';
    std::cout << "indep_avg (ms):" << (indep_avg / iterations).count() << '\n';
    std::cout << "dep_avg (ms):" << (dep_avg / iterations).count() << '\n';
    std::cout << "onl_avg (ms):" << (onl_avg / iterations).count() << '\n';
...
static void execute_circuit_bench(T &io, const bool alice,
  ...
  CircuitType circuits[] = {
      ...
      CircuitType("AES GCM Tag share naive",
      ...
      CircuitType("AES GCM Tag shares Karatsuba",
      ...
      // Verify only the tag
      CircuitType("DiStefano: AES128 Tag Vfy", EmpWrapperAG2PCConstants::AES_GCM_VFY_FILEPATH,
                  "gcm_vfy", EmpWrapperAG2PCConstants::GCM_VFY_INPUT_SIZE,
                  EmpWrapperAG2PCConstants::GCM_VFY_OUTPUT_SIZE),
      // Verify both tag and commit: Surf decrypt
      CircuitType("Surf Decrypt: AES128 Tag Vfy & key commit",
                  EmpWrapperAG2PCConstants::AES_GCM_VFY_COMMIT_FILEPATH,
                  "gcm_vfy_commit",
                  EmpWrapperAG2PCConstants::GCM_VFY_COMMIT_INPUT_SIZE,
                  EmpWrapperAG2PCConstants::GCM_VFY_COMMIT_OUTPUT_SIZE),
      ...
      CircuitType("RMS", EmpWrapperAG2PCConstants::RMS_FILEPATH, "rms",
                  EmpWrapperAG2PCConstants::RESUMPTION_SECRETS_IN_SIZE,
                  EmpWrapperAG2PCConstants::RESUMPTION_SECRETS_OUTPUT_SIZE),
      CircuitType("PSK", EmpWrapperAG2PCConstants::PSK_FILEPATH, "psk",
                  EmpWrapperAG2PCConstants::PSK_SECRETS_IN_SIZE,
                  EmpWrapperAG2PCConstants::PSK_SECRETS_OUTPUT_SIZE),
      CircuitType("PSK 8Byte nonce",
                  EmpWrapperAG2PCConstants::PSK_FILEPATH_8, "psk_8",
                  EmpWrapperAG2PCConstants::PSK_SECRETS_IN_SIZE_8,
                  EmpWrapperAG2PCConstants::PSK_SECRETS_OUTPUT_SIZE),
      CircuitType("PSK 0Byte nonce",
                  EmpWrapperAG2PCConstants::PSK_FILEPATH_0, "psk_0",
                  EmpWrapperAG2PCConstants::PSK_SECRETS_IN_SIZE_0,
                  EmpWrapperAG2PCConstants::PSK_SECRETS_OUTPUT_SIZE),
      CircuitType("PSK Binder", EmpWrapperAG2PCConstants::BINDER_FILEPATH, "binder",
                  EmpWrapperAG2PCConstants::BINDER_IN_SIZE,
                  EmpWrapperAG2PCConstants::BINDER_OUT_SIZE),
      CircuitType("KS batch n=16",
                  EmpWrapperAG2PCConstants::KS_BATCH_FILEPATH, "ks_batch_16",
                  EmpWrapperAG2PCConstants::KS_BATCH_INPUT_SIZE,
                  EmpWrapperAG2PCConstants::KS_BATCH_OUTPUT_SIZE),
      CircuitType("KS batch n=1",
                  EmpWrapperAG2PCConstants::KS_BATCH_1_FILEPATH, "ks_batch_1",
                  EmpWrapperAG2PCConstants::KS_BATCH_1_INPUT_SIZE,
                  EmpWrapperAG2PCConstants::KS_BATCH_1_OUTPUT_SIZE),
      CircuitType("AES-GCM enc batch n=16",
                  EmpWrapperAG2PCConstants::AES_ENC_FILEPATH,
                  "aes_enc_batch_16",
                  EmpWrapperAG2PCConstants::AES_ENC_INPUT_SIZE,
                  EmpWrapperAG2PCConstants::AES_ENC_OUTPUT_SIZE),
  };
