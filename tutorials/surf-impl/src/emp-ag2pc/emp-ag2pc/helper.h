...
block coin_tossing(PRG prg, T* io, int party) {
	...
	if(party == ALICE) {
		...
		if (memcmp(dgst, dgst2, Hash::DIGEST_SIZE)!= 0)
			// PATCHED (surf): was error(...), which exit(1)s the whole
			// process. Throw instead; EmpWrapperAG2PC catches this and
			// fails the run, so a desynced/dead/cheating peer cannot
			// take down a long-lived verifier.
			throw std::runtime_error("cheat CT!");
	}
	io->flush();
	return S ^ S2;
}
