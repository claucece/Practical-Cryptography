...
class SSLTranscript {
 public:
  ...
  // GetTicketNonce writes the ticket nonce used into the bytes
  // pointed by |out| and writes the number of bytes to |*out_len|. |out| must
  // have room for |TODO| bytes. It returns true on success and false
  // on failure.
  bool GetTicketNonce(uint8_t *out, size_t *out_len) const; //TODO CHECK SIGNATURE
...
// surf_capture_ciphertext appends one application record's ciphertext to
// |ssl->surf_records| for later per-block attestation. |body| is the record
// body as it appears on the wire: ciphertext followed by the AEAD tag. Must be
// called before |Open|, which decrypts in place.
...
class SSLKeyShare {
 public:
  ...
  virtual bool Encap(CBB *out_ciphertext, Array<uint8_t> *out_secret,
                     ...
                     Array<uint8_t> *out_other_secret = nullptr) = 0;
...
// tls13_install_3p_traffic_keys installs the raw AES-GCM keys computed by the
// 3-party handshake circuit (stored in ssl->client_key_share,
// ssl->server_key_share, ssl->client_iv, ssl->server_iv) directly into the
// SSL object's AEAD write and read contexts, enabling application data
// encryption/decryption after the 3P handshake.
bool tls13_install_3p_traffic_keys(SSL_HANDSHAKE *hs);
...
struct SSL3_STATE {
  ...
  bool surf_traffic_keys_installed = false;
};
...
// SURF operating mode.
enum class SurfMode : uint8_t { Masked = 0, True = 1, Rotate = 2 };
...
struct ssl_st {
  ...
  // Note; the next set of functions exist solely to make it easier to
  ...
  using derive_handshake_keys_function_type = bool (*) (bssl::SSL_HANDSHAKE *,
                                                        SSL *,bssl::Array<uint8_t>&);

  using derive_resumption_keys_function_type = bool (*) (bssl::SSL_HANDSHAKE *,
                                                        SSL *);

  using derive_psk_keys_function_type = bool (*) (SSL *);

  using derive_binder_function_type = bool (*)(SSL *,
                                               bssl::Span<const uint8_t>,
                                               bssl::Span<uint8_t>);

  using commit_to_session_ticket_function_type = bool (*) (SSL *ssl, const uint8_t *data, size_t len);;

  using derive_traffic_keys_function_type = bool (*) (bssl::SSL_HANDSHAKE *,
                                                      SSL *);
  using commit_to_server_certificate_function_type = bool (*) (bssl::SSL_HANDSHAKE*,
                                                               SSL*);
  using combine_handshake_secret_shares_function_type = bool (*) (bssl::SSL_HANDSHAKE*,
                                                                  SSL*);

  using aes_encrypt_function_type = bool (*) (uint8_t*, size_t*, size_t, uint8_t, uint16_t, uint64_t, bssl::Span<const uint8_t>, const uint8_t*, size_t);
  using aes_decrypt_function_type = bool (*) (bssl::Span<uint8_t>* out, uint8_t, uint16_t, const uint8_t[8], bssl::Span<const uint8_t>, bssl::Span<uint8_t>);

  using derive_gcm_shares_function_type = bool (*)(SSL *);

  using surf_encrypt_function_type = bool (*)(SSL *,
                                              bssl::Span<const uint8_t>,
                                              uint8_t,
                                              bssl::Array<uint8_t> &);

  using surf_decrypt_function_type = bool (*)(SSL *, size_t);

  // ROTATE mode.
  using rotate_traffic_keys_function_type =
      bool (*)(SSL *, enum evp_aead_direction_t);
  ...
  static bool set_derive_handshake_keys(SSL* ssl, derive_handshake_keys_function_type cb);

  static bool set_derive_resumption_keys(SSL* ssl, derive_resumption_keys_function_type cb);

  static bool set_derive_psk_keys(SSL* ssl, derive_psk_keys_function_type cb);

  static bool set_derive_binder(SSL *ssl, derive_binder_function_type cb);

  static bool set_commit_to_session_ticket(SSL* ssl, commit_to_session_ticket_function_type cb);

  static bool set_derive_traffic_keys(SSL * ssl, derive_traffic_keys_function_type cb);

  static bool set_commit_to_server_certificate(SSL* ssl, commit_to_server_certificate_function_type cb);
  static bool set_combine_handshake_secret_shares(SSL* ssl, combine_handshake_secret_shares_function_type cb);

  static bool set_derive_gcm_shares(SSL *ssl,
                                    derive_gcm_shares_function_type cb);

  static bool set_surf_encrypt(SSL *ssl, surf_encrypt_function_type cb);
  static bool set_surf_decrypt(SSL *ssl, surf_decrypt_function_type cb);
  static bool set_rotate_traffic_keys(SSL *ssl,
                                      rotate_traffic_keys_function_type cb);
  ...
  // To measure for the origin server. This is just for measurement, so it can be safely removed.
  std::chrono::steady_clock::time_point flight_t0;
  std::chrono::steady_clock::time_point ch_t0;
  std::chrono::steady_clock::time_point hs_done_t0;

  // Seed for the per-block masks, and the Gamma' commitment key for it.
  // Prover-only and per-session: never serialized, never persisted on the
  // SSL_SESSION. Reusing a seed across sessions makes hiding depend on the
  // traffic IV differing rather than on our own freshness.
  std::array<uint8_t, 16> mask_seed{};
  std::array<uint8_t, 16> mask_seed_key{};

  size_t next_mask_index = 0;

  // SURF TRUE mode. No keystream circuit: records are captured and
  // tag-verified jointly but stay sealed until the verifier releases k_v.
  // Set from SURF_TRUE=1 in preprocess_circuits, before the circuit set is
  // built (do_preproc is interactive OT and must match the verifier).
  SurfMode surf_mode = SurfMode::Masked;
  bool surf_true_mode = false;
  bool surf_key_committed = false;
  bool surf_key_released = false;
  // Randomness for Gamma(r_k, k_c^server); retained so the prover can open at
  // disclosure. Per-session, never persisted on SSL_SESSION.
  std::array<uint8_t, 16> surf_key_commit_rand{};
  // k = k_c ^ k_v, valid only once surf_key_released.
  std::array<uint8_t, 16> surf_full_server_key{};

  // ROTATE mode, incremental release: first record of the epoch not yet
  // released, and the epoch counter.
  size_t surf_epoch_start = 0;
  unsigned surf_epoch = 0;

  // SURF: captured application-record ciphertext for per-block attestation.
  // Each entry is one record: its ciphertext (tag excluded), the tag, and the
  // sequence number the AEAD used, which fixes the GCM counter block.
  struct SurfRecord {
    bssl::Array<uint8_t> ciphertext;
    std::array<uint8_t, 16> tag;
    uint64_t seq;
  };
  std::vector<SurfRecord> surf_records;

  // keystream. e_i = AES.Enc(k_c ^ k_v, ctr_i) for every captured block, in
  // the same global block order as block_masks and surf_block_at. Recovered
  // from the verifier's E_i by XOR-ing our own b_i; see derive_keystream.
  bssl::Array<uint8_t> keystream;
  ...
  derive_resumption_keys_function_type derive_resumption_keys = nullptr;

  derive_psk_keys_function_type derive_psk_keys = nullptr;

  derive_binder_function_type derive_binder = nullptr;

  commit_to_session_ticket_function_type commit_to_session_ticket = nullptr;

  derive_traffic_keys_function_type derive_traffic_keys = nullptr;

  commit_to_server_certificate_function_type commit_to_server_certificate = nullptr;

  combine_handshake_secret_shares_function_type combine_handshake_secret_shares = nullptr;
  ...
  surf_encrypt_function_type surf_encrypt = nullptr;
  surf_decrypt_function_type surf_decrypt = nullptr;
  rotate_traffic_keys_function_type rotate_traffic_keys = nullptr;
  ...
  //IDK if this is the right way to do this but we'll try it for now
  bssl::Array<uint8_t> ticket_nonce;

  std::array<uint8_t, 16*1088> cgcm_share;
  std::array<uint8_t, 16*1088> sgcm_share;
  ...
  // Actual (unmasked) application traffic keys computed by XOR-ing the traffic
  // circuit output with the prover's random mask. Used by tls13_install_3p_traffic_keys
  // after GCM attestation (which uses the raw masks in client_key_share / server_key_share).
  ...
  // executed before the TLS handshake starts (for the offline phase).
  ...
  EmpWrapperAG2PC* resumption_circuit = nullptr;
  EmpWrapperAG2PC* aes_joint_circuit = nullptr;
  EmpWrapperAG2PC* aes_split_circuit = nullptr;
  EmpWrapperAG2PC* gcm_circuit = nullptr;
  EmpWrapperAG2PC* psk_circuit = nullptr;
  // 8-byte ticket_nonce PSK circuit (OpenSSL/nginx origins); psk_circuit above
  // is the 1-byte variant (LiteSpeed/BoringSSL). The matching one is chosen at
  // NewSessionTicket time off ticket_nonce.size().
  EmpWrapperAG2PC* psk_circuit_8 = nullptr;
  EmpWrapperAG2PC* psk_circuit_0 = nullptr;
  EmpWrapperAG2PC* binder_circuit = nullptr;
  EmpWrapperAG2PC *gcm_vfy_circuit = nullptr;
  // Per-block keystream circuit: one amortized AES iteration per committed
  // mask. Consumed during attestation (KS_DERIVE), not during the handshake.
  EmpWrapperAG2PC *ks_block_circuit = nullptr;
  // SURF 2PC-AES-GCM encryption (Algorithm 1). aes_enc_circuit covers
  // AES_ENC_N plaintext blocks per invocation; gcm_tag_circuit computes
  // tau = tau_c ^ tau_v ^ AES.Enc(k, J0) once per record. Both are built in
  // ThreePartyHandshake::preprocess_circuits, in lock-step with the verifier's
  // matching pair in Server::do_preproc, and freed in delete_circuits.
  EmpWrapperAG2PC *aes_enc_circuit = nullptr;
  EmpWrapperAG2PC *gcm_tag_circuit = nullptr;
  // ROTATE mode only.
  EmpWrapperAG2PC *rotate_circuit = nullptr;
...
struct ssl_session_st : public bssl::RefCounted<ssl_session_st> {
  ...
  // SURF: The PSK share
  std::array<uint8_t, 16> surf_psk_share = {};
