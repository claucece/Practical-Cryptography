...
class ECKeyShare : public SSLKeyShare {
 public:
  ...
  bool Encap(CBB *out_ciphertext, Array<uint8_t> *out_secret,
             uint8_t *out_alert, Span<const uint8_t> peer_key, Array<uint8_t> *out_other_secret) override {
    // ECDH may be fit into a KEM-like abstraction by using a second keypair's
    // public key as the ciphertext.
    *out_alert = SSL_AD_INTERNAL_ERROR;
    return Generate(out_ciphertext) && Decap(out_secret, out_alert, peer_key, out_other_secret);
  }

  bool Decap(Array<uint8_t> *out_secret, uint8_t *out_alert,
             Span<const uint8_t> ciphertext, Array<uint8_t> *out_other_secret) override {
    assert(group_);
    assert(private_key_);
    *out_alert = SSL_AD_INTERNAL_ERROR;
    ...
    // Compute the affine co-ordinates of |peer_key| * |private_key_|.
    ...
    Array<uint8_t> secret, secret_y;
    if (!secret.InitForOverwrite((EC_GROUP_get_degree(group_) + 7) / 8) ||
        !BN_bn2bin_padded(secret.data(), secret.size(), x.get())) {
      return false;
    }

    // Encode the y-coordinate left-padded with zeros.
    if (out_other_secret) {
      if (!secret_y.InitForOverwrite((EC_GROUP_get_degree(group_) + 7) / 8) ||
          !BN_bn2bin_padded(secret_y.data(), secret_y.size(), y.get())) {
        return false;
      }
      *out_other_secret = std::move(secret_y);
    }
...
class X25519KeyShare : public SSLKeyShare {
 public:
  ...
  bool Encap(CBB *out_ciphertext, Array<uint8_t> *out_secret,
             ...
             Array<uint8_t> *) override {
    // X25519 may be fit into a KEM-like abstraction by using a second keypair's
    // public key as the ciphertext.
    *out_alert = SSL_AD_INTERNAL_ERROR;
    return Generate(out_ciphertext) && Decap(out_secret, out_alert, peer_key, nullptr);
  }

  bool Decap(Array<uint8_t> *out_secret, uint8_t *out_alert,
             Span<const uint8_t> ciphertext, Array<uint8_t> *) override {
    *out_alert = SSL_AD_INTERNAL_ERROR;
...
class X25519Kyber768KeyShare : public SSLKeyShare {
 public:
  ...
  bool Encap(CBB *out_ciphertext, Array<uint8_t> *out_secret,
             uint8_t *out_alert, Span<const uint8_t> peer_key, Array<uint8_t> *) override {
    Array<uint8_t> secret;
