...
bool tls13_post_handshake(SSL *ssl, const SSLMessage &msg) {
  if (msg.type == SSL3_MT_NEW_SESSION_TICKET && !ssl->server) {
    if (ssl->verifier) {
      ...
      fprintf(stderr, "[measurement!] NST at +%.3f ms from HS done "
              "(+%.3f ms from CH), len=%zu\n",
              std::chrono::duration<double, std::milli>(now - ssl->hs_done_t0).count(),
              std::chrono::duration<double, std::milli>(now - ssl->ch_t0).count(),
              CBS_len(&msg.raw));
    }
