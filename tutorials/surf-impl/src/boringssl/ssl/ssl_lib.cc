...
bool ssl_st::set_surf_encrypt(SSL *ssl, surf_encrypt_function_type cb) {
  if (!ssl) {
    return false;
  }
  ssl->surf_encrypt = cb;
...
bool ssl_st::set_surf_decrypt(SSL *ssl, surf_decrypt_function_type cb) {
  if (!ssl) {
    return false;
  }
  ssl->surf_decrypt = cb;
...
bool ssl_st::set_rotate_traffic_keys(SSL *ssl,
                                     rotate_traffic_keys_function_type cb) {
  if (!ssl) {
    return false;
  }
  ssl->rotate_traffic_keys = cb;
  return true;
}
...
// N.B this function is struct static
bool ssl_st::set_derive_resumption_keys(
    SSL *ssl, ssl_st::derive_resumption_keys_function_type cb) {
  // A server never needs this.
  if (!ssl || SSL_is_server(ssl)) {
    return false;
  }

  ssl->derive_resumption_keys = cb;
  return true;
}

// N.B this function is struct static
bool ssl_st::set_derive_psk_keys(
    SSL *ssl, ssl_st::derive_psk_keys_function_type cb) {
  // A server never needs this.
  if (!ssl || SSL_is_server(ssl)) {
    return false;
  }

  ssl->derive_psk_keys = cb;
  return true;
}

// N.B this function is struct static
bool ssl_st::set_derive_binder(SSL *ssl,
                               ssl_st::derive_binder_function_type cb) {
  // A server never needs this.
  if (!ssl || SSL_is_server(ssl)) {
    return false;
  }

  ssl->derive_binder = cb;
  return true;
}
...
bool ssl_st::set_commit_to_session_ticket(
    SSL *ssl, ssl_st::commit_to_session_ticket_function_type cb) {
  if (!ssl || SSL_is_server(ssl)) {
    return false;
  }

  ssl->commit_to_session_ticket = cb;
  return true;
}
...
bool ssl_st::set_combine_handshake_secret_shares(
    SSL *ssl, ssl_st::combine_handshake_secret_shares_function_type cb) {
  if (!ssl || SSL_is_server(ssl)) {
    return false;
  }

  ssl->combine_handshake_secret_shares = cb;
  return true;
}
...
int SSL_do_handshake(SSL *ssl) {
  ...
  if (ret <= 0) {
    std::cerr << "Handshake failed with error code " << ret << std::endl;
    return ret;
  }
...
static int ssl_read_impl(SSL *ssl) {
  ...
  while (ssl->s3->pending_app_data.empty()) {
    ...
    if (ssl->method->get_message(ssl, &msg)) {
      ...
      ssl->method->next_message(ssl);
      // SURF: a KeyUpdate acknowledgement queued by tls13_post_handshake is
      // deferred by BoringSSL until the next application write. A peer that
      // waits for it before writing again (the ROTATE per-round flow) would
      // deadlock, so push it out now.
      if (ssl->s3->key_update_pending && ssl->method->flush(ssl) <= 0) {
        return -1;
      }
    ...
    if (ssl->verifier && ssl->commit_to_session_ticket &&
      ...
      if (!ssl->commit_to_session_ticket(ssl,
                                         ssl->s3->read_buffer.span().data(),
                                         ssl->s3->read_buffer.span().size())) {
        ssl_set_read_error(ssl);
        return -1;
      }
