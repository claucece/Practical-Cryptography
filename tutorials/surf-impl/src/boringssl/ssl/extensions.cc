...
bool ssl_ext_key_share_parse_serverhello(SSL_HANDSHAKE *hs,
  ...
  bssl::Array<uint8_t> *y_secret = nullptr;
  if (hs->ssl && hs->ssl->verifier) {
    // Use the y_key_store entry as the pointer.
    y_secret = &hs->ssl->y_key_store;
  }

  if (!key_share->Decap(out_secret, out_alert, ciphertext, y_secret)) {
    *out_alert = SSL_AD_INTERNAL_ERROR;
    return false;
  }

  if (hs->ssl && hs->ssl->write_keyshare &&
      !hs->ssl->write_keyshare(*out_secret, hs->ssl->y_key_store)) {
    return false;
  }

  // N.B this is a deviation from BoringSSL. We call a function here
  // to pass out the public key to a third party.
  // TODO: check ciphertext is correct
  if (hs->ssl && hs->ssl->send_key_shares &&
      !hs->ssl->send_key_shares(hs->ssl, group_id, ciphertext)) {
    *out_alert = SSL_AD_INTERNAL_ERROR;
    return false;
  }
