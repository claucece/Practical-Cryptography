...
static bool resolve_ecdhe_secret(SSL_HANDSHAKE *hs,
  ...
  if (hints && !hs->hints_requested && hints->key_share_group_id == group_id &&
    ...
    bssl::Array<uint8_t> out_other;
    if (!key_share ||  //
        !CBB_init(ciphertext.get(), 32) ||
        !key_share->Encap(ciphertext.get(), &secret, &alert, peer_key, &out_other) ||
    ...
    if ((ssl->write_keyshare && !ssl->write_keyshare(secret, out_other))) {
      return false;
    }
  }
...
static enum ssl_hs_wait_t do_send_server_hello(SSL_HANDSHAKE *hs) {
  ...
  hs->key_share_ciphertext.Reset();  // TODO: No longer needed, but this seems correct
