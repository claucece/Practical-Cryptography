...
static enum ssl_hs_wait_t do_read_server_hello(SSL_HANDSHAKE *hs) {
  ...
  fprintf(stderr, "[measurement!] SH at +%.3f ms\n",
        std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - ssl->ch_t0).count());
  ssl->flight_t0 = std::chrono::steady_clock::now();
  ...
  if (pre_shared_key.present) {
    ...
    // SURF: lift the prover's PSK share off the offered session before
    // ssl_set_session(ssl, NULL) drops it.
    ssl->psk_share = ssl->session->surf_psk_share;
  ...
  // SURF: on the MPC path the PSK is secret-shared, so hs->secret cannot be
  // computed here: ES and dES are derived inside the handshake circuit from
  // both parties' shares, and tls13_advance_key_schedule never runs (see the
  // callback branch below). Feed zeroes so nothing downstream mistakes a half
  // key for the PSK.
  if (!tls13_init_key_schedule(hs, (ssl->s3->session_reused && !ssl->verifier)
  ...
  } else if (!ssl->derive_shared_secret(hs, ssl, shared_secret) ||
             //!ssl->advance_key_schedule(hs, ssl, dhe_secret) ||
             //!tls13_advance_key_schedule(hs, dhe_secret) ||
             !ssl_hash_message(hs, msg) ||
             // N.B all traffic secrets for the handshake are derived in this
             // call, but they're received in the commitment below.
             !ssl->derive_handshake_keys(hs, ssl, shared_secret)) {
    return ssl_hs_error;
  }

  // If the verifier is set, we need to commit to the next message (in ciphertext form) to obtain
  // the secrets. That message should already be in the ssl method, so we'll grab that and forward it as is.
  // On resumed handshakes the server sends no Certificate, so skip this
  // commitment entirely — otherwise we'd write CERT bytes into the verifier
  // channel that nobody reads, desynchronising the next message.
  if (ssl->verifier && ssl->commit_to_server_certificate &&
      !ssl->s3->session_reused) {
    bool ok = ssl->commit_to_server_certificate(hs, ssl);
      if (!ok) {
      return ssl_hs_error;
    }
  }

  // Resumed handshakes have no Certificate, so commit_to_server_certificate is
  // skipped. But it also carried the verifier's CHTS/SHTS hi-shares back to
  // the prover; without them hs->client_handshake_secret stays empty and
  // tls13_set_traffic_key derives garbage keys -> BAD_DECRYPT on EE. On
  // resumption, pull just the share frame via combine_handshake_secret_shares.
  if (ssl->verifier && ssl->combine_handshake_secret_shares &&
      ssl->s3->session_reused) {
    if (!ssl->combine_handshake_secret_shares(hs, ssl)) {
      return ssl_hs_error;
    }
  }
...
static enum ssl_hs_wait_t do_read_server_finished(SSL_HANDSHAKE *hs) {
  ...
  // N.B. The else-if is intentional: the 3P callback runs first (giving the
  // verifier its shares), and if it succeeds the normal key-schedule
  // advancement also runs so that hs->client_traffic_secret_0 /
  // hs->server_traffic_secret_0 are populated for tls13_set_traffic_key.
  if (ssl->verifier && ssl->derive_traffic_keys &&
    ...
    return ssl_hs_error;
      ...
      !tls13_advance_key_schedule(
          hs, Span(kZeroes, hs->transcript.DigestLen())) ||
...
static enum ssl_hs_wait_t do_complete_second_flight(SSL_HANDSHAKE *hs) {
  ...
  if (!ssl->verifier && !ssl->derive_traffic_keys && !ssl->derive_resumption_keys) {
  ...
  // SURF
  if (ssl->verifier && ssl->derive_resumption_keys) {
    if (!ssl->derive_resumption_keys(hs, ssl)) {
      return ssl_hs_error;
    }
  }

  // Install the circuit-derived application traffic keys so the prover can
  // encrypt/decrypt application data. The derive_traffic_keys callback stored
  // the actual (unmasked) keys in ssl->client_traffic_key / server_traffic_key;
  // tls13_install_3p_traffic_keys wires them into the SSL AEAD write/read state.
  if (ssl->verifier) {
    if (!tls13_install_3p_traffic_keys(hs)) {
      return ssl_hs_error;
    }
  }

  ssl->hs_done_t0 = std::chrono::steady_clock::now();
...
bool tls13_process_new_session_ticket(SSL *ssl, const SSLMessage &msg) {
  ...
  // Commit to encrypted ticket and install traffic keys
  //if (ssl->verifier && ssl->commit_to_session_ticket) {
  //  if (!ssl->commit_to_session_ticket(ssl)) {
  //    return false;
  //  }
  //}
...
UniquePtr<SSL_SESSION> tls13_create_session_with_ticket(SSL *ssl, CBS *body) {
  // Duplicates the session without auth messages
  ...
  // SURF
  if (!ssl->ticket_nonce.CopyFrom(
        bssl::MakeConstSpan(CBS_data(&ticket_nonce), CBS_len(&ticket_nonce)))) {
     return nullptr;
  }

  if (!ssl->verifier && !ssl->derive_psk_keys) {
  ...
  // SURF
  if (ssl->verifier && ssl->derive_psk_keys) {
    if (!ssl->derive_psk_keys(ssl)) {
      return nullptr;
    }

    session->surf_psk_share = ssl->psk_share;
  }
