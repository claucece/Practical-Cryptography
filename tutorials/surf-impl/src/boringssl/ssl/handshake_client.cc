...
static bool ssl_write_client_cipher_list(const SSL_HANDSHAKE *hs, CBB *out,
  ...
  if (hs->max_version >= TLS1_3_VERSION) {
    ...
    // N.B this is a tweak to BoringSSL, in the same spirit as
    // ssl_set_nist_curves above. The 3PH key-derivation circuits are wired for
    // SHA-256 only (TLS_AES_128_GCM_SHA256). If we offer the SHA-384 suite, a
    // server that prefers it (e.g. nginx defaults) negotiates a 48-byte
    // transcript hash the circuits cannot represent, and do_ks fails. So when a
    // verifier is attached we offer ONLY TLS_AES_128_GCM_SHA256, forcing every
    // origin onto the suite the circuits support (or to fail the handshake).
    static const uint16_t kCiphersVerifier[] = {
        TLS1_3_CK_AES_128_GCM_SHA256 & 0xffff,
    };
    ...
    const bssl::Span<const uint16_t> ciphers =
        ssl->verifier
            ? bssl::Span<const uint16_t>(kCiphersVerifier)
...
static enum ssl_hs_wait_t do_start_connect(SSL_HANDSHAKE *hs) {
  ...
  ssl->ch_t0 = std::chrono::steady_clock::now();
...
static enum ssl_hs_wait_t do_read_server_hello(SSL_HANDSHAKE *hs) {
  ...
  bool t = ssl->method->get_message(ssl, &msg);
