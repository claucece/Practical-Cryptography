...
bool tls13_advance_key_schedule(SSL_HANDSHAKE *hs, Span<const uint8_t> in) {
  ...
  if (!hkdf_expand_label(Span(hs->secret), hs->transcript.Digest(),
                         hs->secret, kTLS13LabelDerived,
                         Span(derive_context, derive_context_len),
                         SSL_is_dtls(hs->ssl))) {
    return false;
  }
  bool ret = hkdf_extract_to_secret(hs, hs->transcript, in);
  return ret;
}
...
// TODO: delete this logging eventually
const char* role =
    SSL_is_server(ssl) ? "SERVER" : "CLIENT";

const char* epoch =
    (level == ssl_encryption_early_data)    ? "EARLY" :
    (level == ssl_encryption_handshake)     ? "HS"    :
    (level == ssl_encryption_application)   ? "APP"   : "UNK";

const char* dir =
    (direction == evp_aead_seal) ? "WRITE" : "READ";

char klabel[64], ivlabel[64];
snprintf(klabel, sizeof(klabel), "%s_%s_%s_TRAFFIC_KEY", role, epoch, dir);
snprintf(ivlabel, sizeof(ivlabel), "%s_%s_%s_TRAFFIC_IV",  role, epoch, dir);

// Log key/iv (Span<const uint8_t> is accepted)
if (!ssl_log_secret(ssl, klabel, key) ||
    !ssl_log_secret(ssl, ivlabel, iv)) {
  return false;
}
...
bool tls13_install_3p_traffic_keys(SSL_HANDSHAKE *hs) {
  ...
  // Both AEAD contexts get a placeholder key. The prover holds only masks,
  // never the real application traffic keys: the write path is replaced by
  // surf_encrypt (Algorithm 1) in tls_seal_record, and the read path by the
  // attested keystream (Algorithm 2) in tls_open_record. Neither context is
  // ever used to seal or open a record; they exist so the record layer's
  // encryption-level tracking and sequence numbers behave normally.
  //
  // The IV is stored as 12 bytes at offset 4 in a 16-byte array, and is not
  // secret: both parties derive it from the traffic circuit's public output.
  const size_t key_len = EVP_AEAD_key_length(aead);
  const size_t iv_len = EVP_AEAD_nonce_length(aead);

  Span<const uint8_t> key_placeholder(ssl->client_key_share.data(), key_len);
  Span<const uint8_t> client_iv(ssl->client_iv.data() + 4, iv_len);
  Span<const uint8_t> server_iv(ssl->server_iv.data() + 4, iv_len);

  auto read_aead = SSLAEADContext::Create(evp_aead_open,
                                          ssl_session_protocol_version(session),
                                          session->cipher, key_placeholder, {},
                                          server_iv);
  if (!read_aead) {
    return false;
  }

  auto write_aead = SSLAEADContext::Create(evp_aead_seal,
                                           ssl_session_protocol_version(session),
                                           session->cipher, key_placeholder, {},
                                           client_iv);
  if (!write_aead) {
  ...
  if (!ssl->method->set_write_state(ssl, ssl_encryption_application,
                                    std::move(write_aead), {}) ||
  ...
  // From here on, records on the read path are application data: this gates
  // SURF ciphertext capture in tls_open_record.
  ssl->s3->surf_traffic_keys_installed = true;
...
bool tls13_rotate_traffic_key(SSL *ssl, enum evp_aead_direction_t direction) {
  // SURF ROTATE: joint rotation over shares. The callback installs fresh AEAD
  // state itself so write_sequence resets as on the normal path.
  if (ssl->verifier && ssl->rotate_traffic_keys) {
    return ssl->rotate_traffic_keys(ssl, direction);
  }
...
// SURF: This will only be called if we have not set it already
...
static bool tls13_psk_binder(uint8_t *out, size_t *out_len, SSL *ssl,
                             const SSL_SESSION *session,
                             const SSLTranscript &transcript,
  ...
  // SURF: on the MPC path the PSK is secret-shared, so the binder_key chain
  // below cannot be computed here
  const bool use_mpc = ssl != nullptr && ssl->verifier != nullptr &&
                       ssl->derive_binder != nullptr;
  ...
  // SURF: the binder ships in the clear inside the ClientHello, so the
  // circuit's output is public to both parties.
  if (use_mpc) {
    if (!ssl->derive_binder(ssl, Span(context, context_len),
                            Span(out, context_len))) {
      return false;
    }
    *out_len = context_len;
    return true;
...
bool tls13_write_psk_binder(const SSL_HANDSHAKE *hs,
  ...
  if (!tls13_psk_binder(verify_data, &verify_data_len, ssl, ssl->session.get(),
...
bool tls13_verify_psk_binder(const SSL_HANDSHAKE *hs,
  ...
  if (!tls13_psk_binder(verify_data, &verify_data_len, hs->ssl, session, hs->transcript,
